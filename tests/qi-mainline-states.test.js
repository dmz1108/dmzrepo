...
const gated = strategyMainlineApplyL2StarGate([
  ...
  { theme: '算力AI', familyKey: 'group:算力AI', count: 5, l2VerificationStatus: 'qi' },
  { theme: '半导体', familyKey: 'group:半导体', count: 4, l2VerificationStatus: 'unscanned' },
]);
A(gated.kept.map(x => x.theme).join(',') === '算力AI,半导体', '已扫无明星的医药被硬排除,有明星/未扫候选保留');
A(gated.excluded.length === 1 && gated.excluded[0].reason === 'completed-scan-without-star', '硬闸返回可审计的排除原因');
