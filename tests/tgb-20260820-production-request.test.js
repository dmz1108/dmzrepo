...
const request = fs.readFileSync(path.join(root, 'ops', 'production', 'requests', '2026-08-20-tgb-hunan-write.ps1'), 'utf8');
...
assert(workflow.includes('DREAMERQI_TGB_20260820_PAYLOAD_B64'), 'manual payload must come from the protected production environment');
assert(workflow.includes("$SCRIPT_PATH\" == 'ops/production/requests/2026-08-20-tgb-hunan-write.ps1'"), 'payload upload must be bound to the exact request');
assert(request.includes("$expectedPayloadSha256 = 'c9dae4e0eab76ad9ecbfdb7a2328ad962c9d6d81cdc64480a91fabed973bbfd5'"), 'request must pin the manual payload hash');
assert(request.includes("articleUrl = 'https://www.tgb.cn/a/2upPwvChvWl'"), 'request must pin the official article');
assert(request.includes("imageFile = 'image-01-06.png'"), 'request must pin the selected official image');
assert(request.includes("expectedImageSha256 = '1334a029c37d6148ab0c94b8423f8589118250e71a026e363152bff43dbae9b3'"), 'request must pin the official image hash');
assert(request.includes('const expectedImageLength = 910433'), 'request must pin the official image length');
...
assert(request.includes("const expectedExcludedCodes = ['920344']"), 'request must pin the excluded Beijing Stock Exchange code');
assert(request.includes("['\\u533b\\u836f', 39]"), 'request must pin the medical block');
assert(request.includes("['\\u673a\\u5668\\u4eba', 6]"), 'request must pin the robot block');
assert(request.includes("['\\u5927\\u6d88\\u8d39', 4]"), 'request must pin the consumption block');
assert(request.includes("['PCB', 3]"), 'request must pin the PCB block');
...
console.log('2026-08-20 TGB production request tests passed');
