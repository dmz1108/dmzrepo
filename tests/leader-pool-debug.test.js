...
(async () => {
  ...
  A(src.match(/if \(diagMode\) \{\s*\n\s*try \{\s*\n\s*await strategyMainlineReworkLeaders\(mainlines, isoDay, reworkOpts\);/), '诊断模式完整等待龙头池重构(旧口径路径;三要件日走 ThreeReqReworkAndGate 全集合同,另有专项测试)');
  ...
  A(/strategyMainlineAugmentPrediction\(\s*item,\s*isTodayQuery,\s*isoDay,\s*!diagMode,\s*trendKeyPrefix,\s*\{ attributionByCode: starAttributionByCode \},\s*\)/.test(src)
    && src.includes('}, recordTrend)'), '诊断今天不写 strategyMainlineTrendSamples(recordTrend 贯通,来源前缀化),且明星归属上下文接入增强器');
