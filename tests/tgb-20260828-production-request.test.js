...
const request = fs.readFileSync(path.join(root, 'ops', 'production', 'requests', '2026-08-28-tgb-hunan-write.ps1'), 'utf8');
...
assert(workflow.includes('DREAMERQI_TGB_20260828_PAYLOAD_B64'), 'manual payload must come from the protected production environment');
assert(workflow.includes("$SCRIPT_PATH\" == 'ops/production/requests/2026-08-28-tgb-hunan-write.ps1'"), 'payload upload must be bound to the exact request');
assert(request.includes("$expectedPayloadSha256 = '6755e6a95605b6791de79dfc441c59d1257f442217e11598097cfbefe37cab06'"), 'request must pin the manual payload hash');
assert(request.includes("articleUrl = 'https://www.tgb.cn/a/2uD6GIwPL0s'"), 'request must pin the official article');
assert(request.includes("imageFile = 'image-01-06.png'"), 'request must pin the selected official image');
assert(request.includes("expectedImageSha256 = '8e2f1542914c01777d25014f5b3d77cc86b6434783637d8d335f23392b6eca74'"), 'request must pin the official image hash');
assert(request.includes('const expectedImageLength = 876211'), 'request must pin the official image length');
assert(request.includes('const expectedCount = 81'), 'request must require the 81-stock formal pool');
assert(request.includes('const expectedRawPoolCount = 82'), 'request must pin the 82-row raw terminal pool');
assert(request.includes("const expectedExcludedCodes = ['920895']"), 'request must pin the Beijing Exchange exclusion');
assert(request.includes("['\\u519c\\u4e1a', 12]"), 'request must pin the agriculture block');
assert(request.includes("['AI\\u5e94\\u7528', 8]"), 'request must pin the AI-application block');
assert(request.includes("['\\u673a\\u5668\\u4eba', 5]"), 'request must pin the robot block');
assert(request.includes("['\\u7b97\\u529b', 5]"), 'request must pin the computing-power block');
assert(request.includes("['\\u533b\\u836f', 5]"), 'request must pin the medical block');
assert(request.includes("['\\u5927\\u6d88\\u8d39', 4]"), 'request must pin the consumption block');
assert(request.includes("['\\u623f\\u5730\\u4ea7', 4]"), 'request must pin the real-estate block');
assert(request.includes("['\\u56fd\\u4ea7\\u82af\\u7247', 4]"), 'request must pin the domestic-chip block');
assert(request.includes("['\\u667a\\u80fd\\u7535\\u7f51', 4]"), 'request must pin the smart-grid block');
assert(request.includes("['\\u5316\\u5de5', 4]"), 'request must pin the chemical block');
...
console.log('2026-08-28 TGB production request tests passed');
