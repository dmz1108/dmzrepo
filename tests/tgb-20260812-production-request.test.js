...
const request = fs.readFileSync(path.join(root, 'ops', 'production', 'requests', '2026-08-12-tgb-hunan-write.ps1'), 'utf8');
...
assert(workflow.includes('DREAMERQI_TGB_20260812_PAYLOAD_B64'), 'manual payload must come from the protected production environment');
assert(workflow.includes("$SCRIPT_PATH\" == 'ops/production/requests/2026-08-12-tgb-hunan-write.ps1'"), 'payload upload must be bound to the exact request');
assert(request.includes("$expectedPayloadSha256 = '94c8a9fdfd74d9b00dd76f2caf9012f6cbd645a77a2b1da3e7ac215b7386b61e'"), 'request must pin the manual payload hash');
assert(request.includes("articleUrl = 'https://www.tgb.cn/a/2ucBhKEKOG9'"), 'request must pin the official article');
assert(request.includes("imageFile = 'image-01-06.png'"), 'request must pin the selected official image');
assert(request.includes("expectedImageSha256 = '23ca24c1aeb3017fe441aafa399ff7b31354048e71078420f4c478a988b0118d'"), 'request must pin the official image hash');
assert(request.includes('const expectedImageLength = 863142'), 'request must pin the official image length');
assert(request.includes('const expectedCount = 91'), 'request must require the 91-stock formal pool');
assert(request.includes('const expectedRawPoolCount = 92'), 'request must pin the 92-row raw terminal pool');
assert(request.includes("const expectedExcludedCodes = ['920856']"), 'request must pin the one North Exchange exclusion');
assert(request.includes("['\\u5927\\u6d88\\u8d39', 13]"), 'request must pin the consumer block');
assert(request.includes("['\\u7b97\\u529b', 12]"), 'request must pin the compute block');
assert(request.includes("['\\u5149\\u901a\\u4fe1', 10]"), 'request must pin the optical-communication block');
...
console.log('2026-08-12 TGB production request tests passed');
