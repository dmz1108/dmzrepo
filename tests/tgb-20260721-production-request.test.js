...
const request = fs.readFileSync(path.join(root, 'ops', 'production', 'requests', '2026-07-21-tgb-hunan-write.ps1'), 'utf8');
...
assert(workflow.includes('DREAMERQI_TGB_20260721_PAYLOAD_B64'), 'manual payload must come from the protected production environment');
assert(workflow.includes("$SCRIPT_PATH\" == 'ops/production/requests/2026-07-21-tgb-hunan-write.ps1'"), 'payload upload must be bound to the exact date-bound request');
...
assert(request.includes("const expectedRawPoolCount = 121"), 'request must independently require the 121-stock raw terminal pool');
assert(request.includes('excludedRows.length !== 1'), 'request must require exactly one ST/BSE/new-prefix exclusion');
...
assert(request.includes("'\\u534a\\u5bfc\\u4f53': 40"), 'request must pin the largest official topic-block count');
assert(request.includes("'\\u5176\\u4ed6\\u4e2a\\u80a1': 10"), 'request must pin the final official topic-block count');
...
console.log('2026-07-21 TGB production request tests passed');
