...
// 验证:最大可统计档映射(股价×申报上限)、第三层档位纳入先决、第二层预期/确认明星均要求最大档主动买>=3亿。
...
const sealedBelowAmount = {
  ...sealedStrongMax,
  thresholds: { ...sealedStrongMax.thresholds, '10000000': th(2.8e8, 1.2e8, 2.4e8, 1.1e8) },
};
const sBelowAmount = strategyMainlineStarStatus(sealedBelowAmount);
A(sBelowAmount?.level === 'sealedWeak' && sBelowAmount.label === '涨停但最大档主动买不足3亿', '封板比值达标但最大档主动买<3亿→不是明星确认');
...
// 9. 最大档字段缺失(worker 未采集)= 数据不完整,不能用小档回退确认
...
A(sMissing?.level === 'sealedWeak' && sMissing.label === '涨停但最大档数据缺失'
  && sMissing.maxBucket.dataMissing === true, '封板:最大档字段缺失→数据不足,不能回退小档确认');
...
// 10. 旧任务无现价:最高非零档只能兼容展示,不能证明该股自己的允许最大档。
const noPriceStrong = {
  code: '600009', gainPct: 7,
  thresholds: { '500000': goodBucket(), '3000000': goodBucket(),
    '8000000': th(3.6e8, 1.7e8, 2.1e8, 1.0e8) },
};
const sNoPricePre = strategyMainlineStarStatus(noPriceStrong);
A(sNoPricePre?.level === 'active' && sNoPricePre.maxBucket.priceMissing === true, '涨停前无现价→最多资金活跃,不能成为预期明星');
const sNoPriceSealed = strategyMainlineStarStatus({ ...noPriceStrong, gainPct: 10 });
A(sNoPriceSealed?.level === 'sealedWeak' && sNoPriceSealed.label === '涨停但最大档现价缺失', '封板无现价→证据不足,不能确认明星');

// 11. 跨来源任务挂载:KPL 创新药扫描可挂回东财医药卡片,后一次空任务不遮蔽有效结果。
