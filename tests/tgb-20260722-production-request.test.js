...
const request = fs.readFileSync(path.join(root, 'ops', 'production', 'requests', '2026-07-22-tgb-hunan-write.ps1'), 'utf8');
...
assert(workflow.includes('DREAMERQI_TGB_20260722_PAYLOAD_B64'), 'manual payload must come from the protected production environment');
assert(workflow.includes("$SCRIPT_PATH\" == 'ops/production/requests/2026-07-22-tgb-hunan-write.ps1'"), 'payload upload must be bound to the exact date-bound request');
...
assert(request.includes("$expectedPayloadSha256 = '1015192cf9a2c45b759273ee2041f9e3bd6dde291ef4dc65ba0c1eae4f4c8514'"), 'request must pin the manual payload hash');
...
assert(request.includes("expectedImageSha256 = '7f57aceb616fe744525793b6c2dc97471e3d277b3f69706c8f6504749426c3e8'"), 'request must pin the twice-reviewed official image hash');
assert(request.includes('officialArticle.title !== articleTitle'), 'request must verify the official article title');
assert(request.includes("const expectedCount = 46"), 'request must require the 46-stock filtered review pool');
assert(request.includes("const expectedRawPoolCount = 46"), 'request must independently require the 46-stock raw terminal pool');
...
assert(request.includes("'\\u7535\\u529b': 12"), 'request must pin the power topic-block count');
assert(request.includes("'\\u5176\\u4ed6\\u4e2a\\u80a1': 7"), 'request must pin the final official topic-block count');
...
console.log('2026-07-22 TGB production request tests passed');
