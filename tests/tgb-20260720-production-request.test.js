...
const request = fs.readFileSync(path.join(root, 'ops', 'production', 'requests', '2026-07-20-tgb-hunan-write.ps1'), 'utf8');
...
assert(workflow.includes('DREAMERQI_TGB_20260720_PAYLOAD_B64'), 'manual payload must come from the protected production environment');
assert(workflow.includes("$SCRIPT_PATH\" == 'ops/production/requests/2026-07-20-tgb-hunan-write.ps1'"), 'payload upload must be bound to the exact date-bound request');
...
assert(request.includes("$expectedPayloadSha256 = '808264f51913362f64d6989effa4c6cdd2e58606bc1fb97be3e7fc6782f9746d'"), 'request must pin the manual payload hash');
assert(request.includes("expectedImageSha256 = '736c9169f450b4c1819b2d3eaf4f7e1f28315ded7d65e82b88df46b0550f0061'"), 'request must pin the twice-reviewed official image hash');
assert(request.includes('officialArticle.title !== articleTitle'), 'request must verify the official article title');
assert(request.includes("const expectedCount = 53"), 'request must require the Owner-confirmed 53-stock pool');
assert(request.includes("baselineCodes.has('601991')"), 'request must include Datang Power in the terminal-pool gate');
assert(request.includes("baselineCodes.has('603533')"), 'request must include iReader Technology in the terminal-pool gate');
assert(request.includes("!baselineCodes.has('600227')"), 'request must exclude unsealed Chitianhua from the terminal-pool gate');
...
assert(request.includes("nameNormalizationDifferences.length === 1"), 'request must allow only the one known full-width name difference');
...
assert(request.includes('validateStoredBaseline(storedBaseline, expectedCodes)'), 'request must ensure force rebuild did not change the Owner-confirmed pool');
...
console.log('2026-07-20 TGB production request tests passed');
