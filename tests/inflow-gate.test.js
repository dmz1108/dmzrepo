...
const thsItems = [
  ...
  { key: 'ths-out', theme: '流出', netInflowZsType: 5, netInflowMetric: 'ths-dde-big-order-amount', netInflow: 9e8, netInflowZjjlr: -2e8, count: 3 },
  { key: 'ths-sticky', theme: '明星粘性', netInflowZsType: 5, netInflowMetric: 'ths-dde-big-order-amount', netInflow: 9e8, netInflowZjjlr: -2e8, count: 3, hadExpectedStarToday: true },
  { key: 'ths-confirmed', theme: '人工确认', netInflowZsType: 5, netInflowMetric: 'ths-dde-big-order-amount', netInflow: 9e8, netInflowZjjlr: -2e8, count: 3 },
...
const thsGate = strategyMainlineApplyInflowGate(thsItems, { key: 'ths-confirmed', theme: '人工确认' });
A(thsGate.kept.map(item => item.key).join(',') === 'ths-ok,ths-sticky,ths-confirmed',
  '同花顺三腿通过者正常入榜，盘中明星粘性与人工确认仅豁免下榜');
A(thsGate.kept[0].fundDirection.value === 1e8 && thsGate.kept[0].thsEligibilityGate.passed === true,
  '方向提示与准入闸门以两个独立接口字段输出');
A(thsGate.kept.find(item => item.key === 'ths-sticky')?.thsEligibilityGate?.exempted === 'expected-star-sticky'
  && thsGate.kept.find(item => item.key === 'ths-sticky')?.thsEligibilityGate?.passed === false
  && thsGate.kept.find(item => item.key === 'ths-sticky')?.fundDirection?.state === 'outflow',
  '出过明星的同花顺主线在方向转负后仍保留，并如实标记粘性豁免与 outflow');
A(thsGate.kept.find(item => item.key === 'ths-confirmed')?.thsEligibilityGate?.exempted === 'owner-confirm'
  && thsGate.kept.find(item => item.key === 'ths-confirmed')?.thsEligibilityGate?.passed === false,
  '人工确认主线未通过组合闸时保留，并标记 owner-confirm 豁免');
A(thsGate.excluded.find(item => item.key === 'ths-out')?.reason === 'ths-net-outflow',
  '无明星、无人工确认的 zjjlr<0 候选仍明确排除为 ths-net-outflow');
