...
const request = fs.readFileSync(path.join(root, 'ops', 'production', 'requests', '2026-08-25-tgb-hunan-write.ps1'), 'utf8');
...
assert(workflow.includes('DREAMERQI_TGB_20260825_PAYLOAD_B64'), 'manual payload must come from the protected production environment');
assert(workflow.includes("$SCRIPT_PATH\" == 'ops/production/requests/2026-08-25-tgb-hunan-write.ps1'"), 'payload upload must be bound to the exact request');
assert(request.includes("$expectedPayloadSha256 = 'da828edb16f9045181f06a89a446b3601bce76183f5d57799ee04d8d730914a1'"), 'request must pin the manual payload hash');
assert(request.includes("articleUrl = 'https://www.tgb.cn/a/2uy7Jgoa8l6'"), 'request must pin the official article');
assert(request.includes("imageFile = 'image-01-06.png'"), 'request must pin the selected official image');
assert(request.includes("expectedImageSha256 = '44ad0bdd1001811e65b0645e0bf99f885807dad494b1f8fa631e4a029cfdb6e4'"), 'request must pin the official image hash');
assert(request.includes('const expectedImageLength = 719980'), 'request must pin the official image length');
assert(request.includes('const expectedCount = 65'), 'request must require the 65-stock formal pool');
assert(request.includes('const expectedRawPoolCount = 65'), 'request must pin the 65-row raw terminal pool');
assert(request.includes('const expectedExcludedCodes = []'), 'request must pin the empty excluded-code set');
assert(request.includes("['\\u533b\\u836f', 9]"), 'request must pin the medical block');
assert(request.includes("['\\u5927\\u6d88\\u8d39', 6]"), 'request must pin the consumer block');
assert(request.includes("['\\u6db2\\u51b7', 5]"), 'request must pin the liquid-cooling block');
assert(request.includes("['\\u56fd\\u4ea7\\u82af\\u7247', 5]"), 'request must pin the domestic-chip block');
assert(request.includes("['\\u673a\\u5668\\u4eba', 4]"), 'request must pin the robot block');
...
console.log('2026-08-25 TGB production request tests passed');
