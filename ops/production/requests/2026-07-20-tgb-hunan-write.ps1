# DreamerQi production operation: write the manually transcribed 2026-07-20
# official @TGB Hunan table, rebuild the same-day combined reason database,
# and verify the public source view.
#
# The 53 formal rows are supplied only through a date-bound encrypted GitHub
# production-environment secret. They are not tracked in Git or printed here.
# The script verifies the payload SHA-256, requires the current Owner-confirmed
# terminal pool (601991/603533 included, 600227 excluded), repeats the complete
# quality gate, backs up every touched runtime artifact, writes atomically,
# rolls back on any failure, and never restarts a service.
...
$expectedPayloadSha256 = '808264f51913362f64d6989effa4c6cdd2e58606bc1fb97be3e7fc6782f9746d'
...
const articleUrl = 'https://www.tgb.cn/a/2tAptm3XkHu';
const articleTitle = '7.20\u6e56\u5357\u4eba\u6da8\u505c\u590d\u76d8+\u665a\u95f4\u6d88\u606f\u6c47\u603b';
const imageFile = 'image-01-06.png';
const imageUrl = 'https://image.tgb.cn/img/2026/07/20/z24ph4hon8kl.png_760w.png';
const expectedImageLength = 694480;
const expectedImageSha256 = '736c9169f450b4c1819b2d3eaf4f7e1f28315ded7d65e82b88df46b0550f0061';
const operationId = 'tgb-hunan-manual-20260720';
...
function validatePayload(payload, baselineRows) {
  ...
  const nameNormalizationOk = nameNormalizationDifferences.length === 1
    && nameNormalizationDifferences[0].code === '000539'
    && nameNormalizationDifferences[0].sourceName === '\u7ca4\u7535\u529bA'
    && nameNormalizationDifferences[0].baselineName === '\u7ca4\u7535\u529b\uff21';
  ...
  const ownerPoolRuleOk = baselineCodes.has('601991')
    && baselineCodes.has('603533')
    && !baselineCodes.has('600227');
  ...
  if (baselineCodes.size !== expectedCount || codeSet.size !== expectedCount || missingCodes.length
      ...
      || topicCountTotal !== expectedCount || !topicCountsMatch || !ownerPoolRuleOk || !nameNormalizationOk) {
...
async function verifyPublic(expectedCodes) {
  ...
  for (let attempt = 1; attempt <= 6; attempt += 1) {
    ...
    const ok = tgbRows.length === expectedCount
      ...
      && Number(statusDay?.count) === expectedCount
...
(async () => {
  ...
  finalPayload.validation = { ...gate, manualSecondPassReviewed: true, ownerPoolDecision: 'include 601991/603533; exclude unsealed 600227' };
  ...
  try {
    ...
    const logEntry = [
      ...
      '- Owner pool decision: include 601991 and 603533; exclude unsealed 600227.',
      `- Formal rows: ${expectedCount}; missing/extra/duplicate/weak/name mismatch: 0/0/0/0/0.`,
      `- Topic counts: 18 + 7 + 6 + 4 + 3 + 15 = ${expectedCount}.`,
