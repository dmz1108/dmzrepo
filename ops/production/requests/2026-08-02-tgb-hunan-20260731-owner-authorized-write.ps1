# DreamerQi production operation: write the manually transcribed 2026-07-31
# official @TGB Hunan table with two owner-authorized clipped-field completions,
# rebuild the same-day combined reason database, and verify the public source view.
#
# The 98 formal rows are supplied only through a date-bound encrypted GitHub
# production-environment secret. They are not tracked in Git or printed here.
# The script verifies the payload, one-time owner authorization, official-image
# SHA-256, and all 98 review-eligible terminal-pool rows before backup/write.
# It writes atomically, rolls back on failure, and never restarts a service.

$ErrorActionPreference = 'Stop'

$project = 'C:\PandaDashboard'
$payloadFile = [string]$env:DREAMERQI_TGB_MANUAL_PAYLOAD_FILE
$expectedPayloadSha256 = 'fa211bd37a1ef1c76498b92e791bff6d19111f7ea3de8e53e28dcc88045b1054'
...
const articleUrl = 'https://www.tgb.cn/a/2tSJcjNYab7';
const articleTitle = '7.31\u6e56\u5357\u4eba\u6da8\u505c\u590d\u76d8+\u665a\u95f4\u6d88\u606f\u6c47\u603b';
const imageFile = 'image-01-06.png';
const imageUrl = 'https://image.tgb.cn/img/2026/07/31/10g3ru3rhcqj.png_760w.png';
const expectedImageLength = 1505003;
const expectedImageSha256 = '3048e335c7741d45f7231a5d44227006834b9a40bf99d85aa3ed60ff518726a3';
const expectedCount = 98;
const expectedRawPoolCount = 99;
const expectedEligibleCodeSetSha256 = '908743e0babcb01990c299e4b3af97774d988811f24982b382e28eff814d678f';
const operationId = 'tgb-hunan-owner-authorized-manual-20260731';
...
const fixedOverrideQualityNote = 'Manual transcription from @TGB\u6e56\u5357\u4eba official table image; owner-authorized completion for right-edge-clipped detailReason on 2026-08-02.';
...
const expectedManualOverrides = [
  {
    code: '605178',
    field: 'detailReason',
    visiblePrefix: '\u62df\u5e76\u8d2d\u5609\u5408\u52b2\u5a01\uff08\u5b58\u50a8\u82af\u7247',
    authorizedCompletion: '\uff09',
    finalValue: '\u62df\u5e76\u8d2d\u5609\u5408\u52b2\u5a01\uff08\u5b58\u50a8\u82af\u7247\uff09',
    authorizedBy: 'owner',
    authorizedAt: '2026-08-02',
    scope: 'one-time-2026-07-31',
  },
  {
    code: '605198',
    field: 'detailReason',
    visiblePrefix: '\u62df\u6536\u8d2d\u5b81\u6ce2\u752c\u5f3a\u79d1\u6280\uff08\u8986\u94dc',
    authorizedCompletion: '\u677f\uff09',
    finalValue: '\u62df\u6536\u8d2d\u5b81\u6ce2\u752c\u5f3a\u79d1\u6280\uff08\u8986\u94dc\u677f\uff09',
    authorizedBy: 'owner',
    authorizedAt: '2026-08-02',
    scope: 'one-time-2026-07-31',
  },
];
const expectedOverrideCodes = new Set(expectedManualOverrides.map(item => item.code));
...
const normalizeName = value => asText(value).normalize('NFKC').replace(/\s+/g, '');
const toSet = values => new Set(values);
const setDiff = (left, right) => [...left].filter(value => !right.has(value)).sort();
const stableJson = value => JSON.stringify(value);

function expectedNameAlias(code, sourceName, baselineName) {
  return expectedNameDifferences.some(item => item.code === code
    && item.sourceName === sourceName
    && item.baselineName === baselineName
    && item.normalization === 'explicit-source-alias');
}
...
function validateBaseline(payload) {
  ...
  const eligibleCodeSetSha256 = shaBuffer(Buffer.from([...eligibleCodes].sort().join('\n'), 'utf8'));
  if (rawRows.length !== expectedRawPoolCount
      ...
      || eligibleCodeSetSha256 !== expectedEligibleCodeSetSha256) {
...
function validatePayload(payload, baselineGate) {
  ...
  const weakRows = rows.filter(row => ![
    ...
    || row.qualityNote !== (expectedOverrideCodes.has(normalizeCode(row.code))
      ? fixedOverrideQualityNote
      : fixedQualityNote));
  ...
  const nameDifferences = rows
    ...
    .map(row => {
      ...
      const sourceName = asText(row.name);
      const baselineName = asText(baselineByCode.get(code));
      return {
        code,
        sourceName,
        baselineName,
        normalization: normalizeName(sourceName) === normalizeName(baselineName)
          ? 'NFKC+remove-whitespace'
  ...
  const nameMismatches = rows
    .filter(row => {
      ...
      const sourceName = asText(row.name);
      const baselineName = asText(baselineByCode.get(code));
      return normalizeName(sourceName) !== normalizeName(baselineName)
        && !expectedNameAlias(code, sourceName, baselineName);
    })
    .map(row => normalizeCode(row.code));
  const nameDifferencesMatch = stableJson(nameDifferences) === stableJson(expectedNameDifferences);
  ...
  const manualBlocks = Array.isArray(payload.manualBlocks) ? payload.manualBlocks : [];
  const manualBlockTotal = manualBlocks.reduce((sum, block) => sum + Number(block.manualRowCount || 0), 0);
  const manualBlocksMatch = manualBlocks.length === expectedManualBlocks.length
    && expectedManualBlocks.every(([topic, count], index) => Number(manualBlocks[index]?.sequence) === index + 1
      && asText(manualBlocks[index]?.boardTopic) === topic
      && Number(manualBlocks[index]?.manualRowCount) === count);
  const manualOverrides = Array.isArray(payload.manualOverrides) ? payload.manualOverrides : [];
  const manualOverridesMatch = stableJson(manualOverrides) === stableJson(expectedManualOverrides);
  const overrideRowsMatch = expectedManualOverrides.every(expected => {
    const row = rows.find(candidate => normalizeCode(candidate.code) === expected.code);
    return row
      && asText(row.detailReason) === expected.finalValue
      && row.qualityNote === fixedOverrideQualityNote;
  });
  const gate = {
    ...
    nameMismatches,
    nameDifferences,
    nameDifferencesMatch,
    topicCounts,
    topicCountTotal,
    manualBlocks,
    manualBlockTotal,
    manualOverrides,
    manualOverridesMatch,
    overrideRowsMatch,
  };
  if (baselineCodes.size !== expectedCount
      ...
      || !topicCountsMatch
      || !manualBlocksMatch
      || manualBlockTotal !== expectedCount
      || !manualOverridesMatch
      || !overrideRowsMatch) {
...
(async () => {
  ...
  finalPayload.validation = {
    ...
    ownerAuthorizedOverrides: expectedManualOverrides,
  };
  ...
  try {
    ...
    const logEntry = [
      ...
      `- Manual payload: count=${expectedCount}; sha256=${inputSha256}; second-pass-reviewed=true; owner-authorized-overrides=2`,
      `- Gate: missing=0; extra=0; duplicates=0; weak=0; name-mismatches=0; explicit-name-differences=${gate.nameDifferences.length}; topic-total=${gate.topicCountTotal}; manual-block-total=${gate.manualBlockTotal}; owner-override-codes=605178,605198`,
      `- Backup: ${backupDir}`,
