# Read-only terminal-pool inspection for the 2026-09-24 @TGB Hunan review.
# Prints only the filtered public stock-code/name set and gate metadata.
$ErrorActionPreference = 'Stop'
[Console]::OutputEncoding = [System.Text.UTF8Encoding]::new($false)
$project = 'C:\PandaDashboard'
$day = '2026-09-24'
$expectedEligibleCount = 51
$baselineFile = Join-Path $project 'kpl-limitup-db\2026-09-24.json'
...
if ($rawRows.Count -ne $rawCodes.Count) { throw ('Duplicate terminal-pool codes: rows=' + $rawRows.Count + '; unique=' + $rawCodes.Count) }
