# DreamerQi production operation: write the manually transcribed 2026-07-23
...
$expectedPayloadSha256 = '934b778a239e9010e89a11bb12e7304de3ba4877e2dd55d74be7dd94be5b5269'
...
const articleUrl = 'https://www.tgb.cn/a/2tFsEMDE2fP';
const articleTitle = '7.23\u6e56\u5357\u4eba\u6da8\u505c\u590d\u76d8+\u665a\u95f4\u6d88\u606f\u6c47\u603b';
const imageFile = 'image-01-06.png';
const imageUrl = 'https://image.tgb.cn/img/2026/07/23/yk1ocfjuge9b.png_760w.png';
const expectedImageLength = 1170327;
...
function validatePayload(payload, baselineRows) {
  ...
  if (baselineCodes.size !== expectedCount || codeSet.size !== expectedCount || missingCodes.length
      ...
      || topicCountTotal !== expectedCount || !topicCountsMatch || !manualBlocksMatch
      || manualBlockTotal !== expectedCount || !nameNormalizationOk) {
...
(async () => {
  ...
  payload.rows = (Array.isArray(payload.rows) ? payload.rows : []).map(row => ({
    ...row,
    source: fixedSource,
    matchType: 'manual-hunan-table',
    reasonQuality: 'clear',
    confidence: 0.99,
    qualityNote: fixedQualityNote,
  }));
  ...
  try {
    ...
    const logEntry = [
      ...
      `- Topic-block counts: 34 + 10 + 9 + 8 + 8 + 6 + 5 + 4 + 4 + 3 + 3 + 3 + 3 + 3 + 3 + 2 + 7 = ${expectedCount}.`,
