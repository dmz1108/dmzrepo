# Date-bound, raw-only @TGB Hunan evidence refresh for 2026-09-24.
...
$priorDir = Join-Path $rawRoot '2026-09-23'
$formalFile = Join-Path $project 'kpl-limitup-main-reason-sources\tgb-hunan-structured\2026-09-24.json'
$logNames = @('panda-cloud-ops-2026-06-19.md', '_cloud-change-log-20260705.md')
$stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$backupDir = Join-Path $project ('backups\tgb-hunan-raw-20260924-' + $stamp)
...
# its upstream calendar has not included Thursday yet; the day check then stops.
if (Test-Path -LiteralPath $priorDir) { Copy-Item -LiteralPath $priorDir -Destination (Join-Path $backupDir '2026-09-23') -Recurse -Force }
foreach ($name in $logNames) {
...
if ($articles.Count -eq 0) {
  ...
  foreach ($article in $articles) {
    if ([string]$article.url -notmatch '^https://www\.tgb\.cn/a/' -or
        -not ([string]$article.title).StartsWith('9.24', [System.StringComparison]::Ordinal) -or
...
$logEntry = @(
  ''
  ('## 2026-09-24 - tgb-hunan-raw-evidence-' + $stamp)
