# DreamerQi production operation: write the manually transcribed 2026-07-21
# official @TGB Hunan table, rebuild the same-day combined reason database,
# and verify the public source view.
#
# The 120 formal rows are supplied only through a date-bound encrypted GitHub
# production-environment secret. They are not tracked in Git or printed here.
# The script verifies the payload SHA-256, reconciles the 120 review-eligible
# rows against the current 121-row raw terminal pool after excluding the one
...
$expectedPayloadSha256 = 'b8e7a9c2c07e64e88e088ad366aa219179feb973631a070390bcf35272176ade'
...
const articleUrl = 'https://www.tgb.cn/a/2tC8FxwG90V';
const articleTitle = '7.21\u6e56\u5357\u4eba\u6da8\u505c\u590d\u76d8+\u665a\u95f4\u6d88\u606f\u6c47\u603b';
const imageFile = 'image-01-06.png';
const imageUrl = 'https://image.tgb.cn/img/2026/07/21/expg7tsek9kl.png_760w.png';
const expectedImageLength = 1021460;
const expectedImageSha256 = '88318532abb5e8d438cd17071e0e9ff66730d1f8aeaa007aacb752a4716c55cb';
const operationId = 'tgb-hunan-manual-20260721';
const expectedCount = 120;
const expectedRawPoolCount = 121;
...
function validateRawBaseline(payload) {
  ...
  if (rows.length !== expectedRawPoolCount || codes.size !== expectedRawPoolCount || excludedRows.length !== 1) {
...
(async () => {
  ...
  try {
    ...
    const logEntry = [
      ...
      `- Topic counts: 40 + 11 + 11 + 7 + 7 + 5 + 4 + 4 + 4 + 3 + 3 + 3 + 3 + 3 + 2 + 10 = ${expectedCount}.`,
