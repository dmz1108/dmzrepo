...
$priorDir = Join-Path $rawRoot '2026-09-18'
$formalFile = Join-Path $project 'kpl-limitup-main-reason-sources\tgb-hunan-structured\2026-09-21.json'
$logNames = @('panda-cloud-ops-2026-06-19.md', '_cloud-change-log-20260705.md')
$stamp = Get-Date -Format 'yyyyMMdd-HHmmss'
$backupDir = Join-Path $project ('backups\tgb-hunan-raw-20260921-' + $stamp)
...
# The CLI uses a trading-day window. Back up Friday as well in case its upstream
# calendar has not included Monday yet; the result-day check below then stops.
if (Test-Path -LiteralPath $priorDir) { Copy-Item -LiteralPath $priorDir -Destination (Join-Path $backupDir '2026-09-18') -Recurse -Force }
foreach ($name in $logNames) {
...
if ($articles.Count -eq 0) {
  ...
  foreach ($article in $articles) {
    if ([string]$article.url -notmatch '^https://www\.tgb\.cn/a/' -or
        -not ([string]$article.title).StartsWith('9.21', [System.StringComparison]::Ordinal) -or
...
$logEntry = @(
  ''
  ('## 2026-09-21 - tgb-hunan-raw-evidence-' + $stamp)
