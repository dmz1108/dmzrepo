# Date-bound, raw-only @TGB Hunan evidence refresh for 2026-09-22.
...
$priorDir = Join-Path $rawRoot '2026-09-21'
...
$backupDir = Join-Path $project ('backups\tgb-hunan-raw-20260922-' + $stamp)
...
# its upstream calendar has not included Tuesday yet; the day check then stops.
if (Test-Path -LiteralPath $priorDir) { Copy-Item -LiteralPath $priorDir -Destination (Join-Path $backupDir '2026-09-21') -Recurse -Force }
foreach ($name in $logNames) {
...
if ($articles.Count -eq 0) {
  ...
  foreach ($article in $articles) {
    if ([string]$article.url -notmatch '^https://www\.tgb\.cn/a/' -or
        -not ([string]$article.title).StartsWith('9.22', [System.StringComparison]::Ordinal) -or
...
$logEntry = @(
  ''
  ('## 2026-09-22 - tgb-hunan-raw-evidence-' + $stamp)
