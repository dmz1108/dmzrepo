...
实施前必须先在公司电脑确认：实际运行脚本路径、启动方式、计划任务/守护进程、日志路径和当前配置来源。先备份实际脚本，再修改。
...
1. 从最新 `main` 建立 `company/l2-worker-five-buckets-YYYYMMDD` 分支。
2. 在公司电脑确认实际 worker 脚本和启动配置；记录路径但不要把本地密钥提交 Git。
3. 备份实际 worker、配置和启动方式。
4. 保留现有已验证的 AXTICK 登录、下载、金额单位换算和方向字段映射。
5. 将汇总档位统一为 `500000/3000000/5000000/8000000/10000000`。
6. 对完整逐笔数据先按买卖双方委托号聚合，再计算五档。
7. 获取并回传人民币元现价 `price`；原样保留 job 的 `code/name/gainPct/rank/batch`。
8. 按 `job.stocks` 顺序扫描；每次发送累计结果；完成时覆盖全部股票。
9. claim 与 result 都回传 `version: "company-l2-worker/2.0.0"`。
10. 先离线验收，再连接云端单任务联调；通过前不要替换正式常驻 worker。
...
任一条件失败都不能替换正式 worker。

## 10. 回退

- 保留升级前 worker 和启动配置备份。
