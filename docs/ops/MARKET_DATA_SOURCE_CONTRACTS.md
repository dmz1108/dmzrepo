...
**Directionality (2026-07-17 three-way calibration): field `527198` must be treated as an unsigned DDE big-order activity magnitude. It cannot be interpreted as a signed net inflow.**

- Evidence: 2026-07-17 intraday, 国资云 showed desktop-client DDE **−5亿**, `zjjlr` **−3.15亿**, and a falling board price — while `527198` read **+5.54亿**; 智慧政务 showed client **−13亿** vs `527198` **+13.17亿**. Three independent signals agree the boards were bleeding; `527198` stayed positive. Across all boards and stocks verified so far (including 烽火通信 at −6.8% on heavy volume), `527198` has been non-negative. Until an official field definition or a negative counterexample appears, treat it as a directionless magnitude.
...
- No verifiable public field has been found that reproduces the signed desktop-client DDE column: the `264648`-based formulas fit some boards and are falsified by others (`|264648|` exceeded 100% on 手机游戏 2026-07-17), and the buy/sell pairs do not reconcile. The signed column is presumed to come from an authenticated channel; this remains unproven.
- Naming rule: fields and UI labels for this metric must say "DDE 大单金额/活跃度", never "净流入". The signed `zjjlr` remains available in `netInflowZjjlr` for audit and direction display; the two metrics are never mixed in one column.
- **Known gating gap (production behavior until the composite gate ships)**: with `527198` feeding the THS-side `netInflow >= 5亿` auto-scan leg alone, that leg is direction-blind — a clearly net-outflow board can consume scan quota. 涨停≥2 and the L2 star gate are price/stock-level evidence and do **not** certify board-level net inflow, so they are not direction backstops.
- **Composite THS eligibility (APPROVED by Owner 2026-07-20; implementation by Codex on a separate branch, reviewed by Claude)**: `527198 >= 5亿` (DDE big-order activity) AND `netInflowZjjlr > 0` (direction) AND `涨停 >= 2`, followed by the unchanged L2 expected/confirmed star requirement. A direction hint derived from `zjjlr` sign accompanies it on cards, and `zjjlr <= 0` also blocks THS auto-scan and formal-mainline eligibility. Implementation note (Codex co-sign): the direction *hint* and the direction *gate* must be exposed as two distinct result fields in interface metadata — the same `zjjlr` sign may drive both, but their purposes stay separate so "display-only" is never conflated with eligibility.
- Signed-source search: **parked, not closed** (Codex co-sign 2026-07-20). Reopen triggers: a high-magnitude `zjjlr < 0` board actually entering scans/candidates and causing a misjudgment; a stable authenticated/paid same-metric interface becoming available; or new simultaneous client samples from the Owner. Constituent self-aggregation (方案 H) may run only as experimental verification until membership coverage, dedup, and same-timestamp checks pass.
