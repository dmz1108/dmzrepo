...
- **Known gating gap (current production behavior, NOT an endorsed end state)**: with `527198` feeding the THS-side `netInflow >= 5亿` auto-scan leg, that leg is direction-blind — a clearly net-outflow board can consume scan quota. 涨停≥2 and the L2 star gate are price/stock-level evidence and do **not** certify board-level net inflow, so they are not direction backstops. This conflicts with the Owner rule "资金净流出不能成为当日主线" and needs an explicit Owner decision.
- Proposed composite THS eligibility (Codex co-sign 2026-07-20, pending Owner confirmation, code change in a separate PR): `527198 >= 5亿` (activity) AND `netInflowZjjlr > 0` (direction) AND `涨停 >= 2`, followed by the unchanged L2 expected/confirmed star requirement. A display-only direction hint from `zjjlr` sign accompanies it; `zjjlr <= 0` would also block THS auto-scan and formal-mainline eligibility.
