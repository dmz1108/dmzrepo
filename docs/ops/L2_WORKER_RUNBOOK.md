...
Automatic dispatch keeps the existing per-board eligibility gate and serial worker contract. It ranks eligible work as:

1. expected-star seal confirmation;
2. retry after an explicit worker error;
3. first discovery scan;
4. strengthening rescan.

Within discovery and strengthening, a family with fewer already-scanned boards receives the next opportunity first. Among strengthening rescans, the lower scan sequence wins before fund-flow and gain tie-breakers. This prevents one repeatedly strengthening board from occupying the worker while another eligible board or family has never been scanned.
