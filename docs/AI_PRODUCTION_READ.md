...
历史验收示例使用 20 日窗口，因为业务底库只滚动保留最近 30 个交易日；较早目标日再向前请求完整 30 日时，最早几日可能已按保留规则清理，此时接口会正确返回 `complete:false` 和具体缺失日期。分析当前交易日时仍可使用 `--window=30`。
...
`complete:true` 要求历史窗口中的每个必要交易日都具备涨停库、主因库和收盘价库，而不只是目标日存在。整包 SHA-256 不包含抓取时间 `generatedAt`，因此相同历史证据重复抓取时保持稳定，但仍覆盖 `complete`、缺失源和读取错误。它不是数字签名，只有把 `bundleSha256` 独立记录在 PR/交接中并在回放时使用 `--expect-sha`，才能防止证据包与哈希被一起替换。
