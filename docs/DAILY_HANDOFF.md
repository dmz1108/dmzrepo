...
## 2026-08-27 - Codex - TGB 湖南人正式入库与公网验收回执

Changed:
- PR #484、#485、#486 已依次合并；受保护运行 `33131697844` 强刷官方 raw evidence，
  `33132299731` 只读对账终盘池，`33132976824` 完成76行正式写入与当日综合主因重折。
- 正式写入前已备份同日终盘池、综合/证据/质量/auto 产物、已健康来源、raw manifest/原图与两份
  云端日志；写入后两份云端运维日志已追加安全回执。

Files:
- `docs/DAILY_HANDOFF.md`

Validated:
- 官方文章 `https://www.tgb.cn/a/2uBuay3MSqq`，原图 `image-01-06.png`，530×4736、801096字节，
  SHA-256 `18e03a34538b496c7982d6c6c52a78d694ade3b45e9f86ea262a264d2b5ebf2c`。
- 官方原图人工双遍复核76行：光通信15、农林牧渔8、PCB6、大金融5、半导体4、氟化工4、
  液冷服务器4、医疗医药4、存储3、黄金3、业绩增长3、其他热点12、其他个股5；市场连板股16行为
  摘要重复，涨停炸板17行排除。
- 终盘原始池77/77，排除北交所 `920895` 后76/76；人工候选与落盘后二次闸均为
  `missing=[]`、`extra=[]`、重复0、`weak=0`、题材合计76、实质名称不匹配0；仅 `301666`
  大普微/大普微-UW、`688790` 昂瑞微/昂瑞微-UW 为显式来源别名。
- 正式 TGB SHA-256 `639ea94ed0067c903f6248498c9eb5e97e76fc87653769db3202f753dbf23a24`；
  重折综合 SHA-256 `a1a7b40a9caaa38135be157d83f9adb257cf9e09d98d43b15d9fafa275aee4fe`。
- 独立公网复验：综合归纳、复盘啦、选股宝、韭研、淘股吧均76/76；TGB 覆盖率/主因覆盖率100%，
  低质量0，`sourceErrors=[]`，`/health` 为 `ok:true`。

Deployment:
- 生产数据已写入并重折当日综合主因；备份路径
  `C:\\PandaDashboard\\backups\\tgb-hunan-manual-20260827-20260828012933`。
- 未部署服务代码，未重启主服务或其他服务。

Notes for next agent:
- 2026-08-27 正式 TGB 文件已受人工来源保护；普通同步和 `force` 不得覆盖。若需更正，须重做备份、
  全量对账、重折和公网验收。
