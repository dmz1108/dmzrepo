...
## 2026-08-24 - Codex - TGB 湖南人正式入库与公网验收回执

Changed:
- PR #472、#473、#474 已依次合并；受保护运行 `32722742773` 强刷官方 raw evidence，
  `32723903119` 只读对账终盘池，`32725796516` 完成46行正式写入与当日综合主因重折。
- 正式写入前已备份同日终盘池、综合/证据/质量/auto 产物、已健康来源、raw manifest/原图与两份云端日志；
  写入后两份云端运维日志已追加安全回执。
...
Validated:
- 官方文章 `https://www.tgb.cn/a/2uwthn6EfR5`，原图 `image-01-06.png`，530×3366、559319字节，
  SHA-256 `ca7433198be529fa7c75567e501b88e6d59aece70dde1c5b2f2b3b26e46f464f`。
- 官方原图人工双遍复核46行：医药6、锂电池4、半导体3、电力3、黄金3、机器人3、煤炭3、算力3、
  其他热点14、其他个股4；市场连板股10行为摘要重复，涨停炸板16行排除。
- 终盘原始池/过滤池均46/46；人工候选与落盘后二次闸均为 `missing=[]`、`extra=[]`、重复0、
  `weak=0`、题材合计46、实质名称不匹配0；仅 `000931` 中关村/中 关 村为已声明的去空格差异。
- 正式 TGB SHA-256 `93d42a668e0eab3b2d36b7bf4190dfc4ba959a398ef63b6daa86dae60ab4dd14`；
  重折综合 SHA-256 `9ed10bdbe83355e7b77e610c16f6e2eb744168c09fce6adba5ceefe5618ecec4`。
- 独立公网复验：综合归纳、复盘啦、选股宝、淘股吧均46，韭研0；TGB覆盖率/主因覆盖率100%，
  低质量0，`sourceErrors=[]`，`/health` 为 `ok:true`。

Deployment:
- 生产数据已写入并重折当日综合主因；备份路径
  `C:\\PandaDashboard\\backups\\tgb-hunan-manual-20260824-20260824121318`。
- 未部署服务代码，未重启主服务或其他服务。

Notes for next agent:
- 2026-08-24 正式 TGB 文件已受人工来源保护；普通同步和 `force` 不得覆盖。若需更正，须重做备份、
  全量对账、重折和公网验收。
