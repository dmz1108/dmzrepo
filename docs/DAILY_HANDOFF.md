...
## 2026-09-22 - Codex - 东财指数信息故障隔离与日期校验

Changed:
- 生产公开指数接口复现 HTTP 500 / fetch failed；云端东财 stock/get 请求遇到 UND_ERR_SOCKET，
  替代主机也不能稳定返回可信数据。旧 Promise.all 让上涨/下跌家数失败拖垮整块信息，涨跌停抓取
  失败还会被转成 0。现改为独立结算：缺失为 null，保留成功项，附状态和逐项日期证据。
- 同日请求合并、结果缓存30秒、最多4个日期；失败可复用原采集时间起90秒以内的同日缓存并标 stale，
  失败不续期。各上游请求最多6秒；只改变指数查询，不改变共享东财请求器、资金口径、策略打分或L2。
- 校验指数 f124 的北京时间日期和板块池 qdate；历史请求不再混入实时家数，盘前显示等待开盘。
  云端实测 09-21 涨跌停请求返回 qdate=20260922，已拒绝将其当作09-21数据。
- 页面显示 -- / 数据暂缺，缓存值带 * 和采集时间；切换日期/来源后旧请求不得覆盖新页面。
  紧凑状态放在指标栏前部，避免手机横向滚动时被裁掉；成功恢复后自动清除提示。

Files:
- `kpl-stats-server.js`
- `kpl-dashboard_17_apple.html`
- `tests/eastmoney-index-info.test.js`
- `docs/DAILY_HANDOFF.md`

Validated:
- `node --check kpl-stats-server.js`、`git diff --check` 通过。
- 新增20项指数故障/日期/缓存/并发/UI回归全部通过；另8套日期锁、实时主因/UI、同花顺性能/DDE、
  策略实时数据、东财超大单口径、K线回看鲜度回归通过。
- Playwright 用本地完整HTML/CSS及实际KPI渲染函数隔离验证，1440和390像素截图通过；
  缺失、真实0、提示不裁切、恢复隐藏、浏览器无脚本错误均通过。非登录生产整站端到端测试。
- 云端通过 node stdin 临时执行实际新指数函数，无文件写入；北京时间09:20验证09-22返回
  HTTP200/pending/全null（盘前不抓取），09-21返回HTTP200/unavailable/日期不匹配原因。
  原始回执仅留本地忽略目录 `tmp/index-cloud-smoke.jsonl`，SHA-256：
  `9fa4eeedc2862ddea9655f8f1822d20db582510663cfff7733c754050f33a84f`。
  安全复核参数：GET `/api/eastmoney-index-info?day=2026-09-21` 及北京时间当天日期，无认证参数。

Deployment:
- 基于 main `7ea037c`，实现分支提交待复审；未合并、未部署、未重启服务、未修改生产运行时文件。
- 云端仅只读诊断，未更新云端操作日志；正式部署时需按流程备份、检查、重启和补写云端日志。

Notes for next agent:
- 上游真实家数尚未证实恢复，不能将 HTTP200 降级响应说成数据完整恢复；须同时检查 complete 和 metrics。
- 开盘后独立验证真实上游日期/数值及同日缓存期限，再由 Owner 确认合并部署；无可靠历史证据保留null，
  不补造历史数字或覆写冻结快照。行情页其他来源、正式策略和L2逻辑保持不动。
