...
## 2026-07-24 - Codex - 准备受保护写入当日 TGB 正式库

Changed:
- 受保护 raw 运行 `30090201261` 强制刷新 2026-07-24 官方文章与 16 张原始图片；只采用标题、日期、白底表格和 `@TGB湖南人` 水印均匹配的 `image-01-07.png`。
- 排除统计折线图、其他作者题材表、行情截图、头像和官方图底部 11 行“涨停炸板”；Codex 按 6 个题材块人工逐行转录 40 行并逐字段二次回看复核，全程未使用 OCR、Qwen 或自动视觉结果。
- 新增日期绑定正式写入脚本、受保护密文上传分支及专项测试；脚本在任何写入前固定复核人工载荷、官方文章/原图哈希、40 股终盘池和全部质量闸，失败则回滚。

Files:
- `.github/workflows/production-ops.yml`
- `ops/production/requests/2026-07-24-tgb-hunan-write.ps1`
- `tests/tgb-20260724-production-request.test.js`
- `docs/DAILY_HANDOFF.md`

Validated:
- 官方文章 `https://www.tgb.cn/a/2tH73JZqkpi`，标题 `7.24湖南人涨停复盘+晚间消息汇总`；官方原图 `image-01-07.png`，长度 530066 字节，SHA-256 `60347f3fcbe837df5007852f8d257b20e0a2375cd60d5386b0c1f5a61b54b88c`。
- 人工载荷 40 行、唯一代码 40；`missingCodes=[]`、`extraCodes=[]`、重复 0、`weakCount=0`、名称差异 0；人工载荷 SHA-256 `e8ec18efa6f7dc81f6be0ff89d8c2049f776f5541b4deee0a10f57a549e55915`。
- 题材块计数：半导体 8、军工 7、智能电网 6、资产重组 3、其他热点 6、其他个股 10，合计 40；顶部 17 行“市场连板股”为重复摘要，底部 11 行“涨停炸板”未进入正式载荷。

Deployment:
- raw evidence 已刷新；正式 TGB 尚未写入、综合主因尚未重折、服务未重启。
...
## 2026-07-24 - Codex - 准备强刷当日 TGB 湖南人原始证据

Changed:
- 新增受保护、日期绑定的一次性生产脚本，用于强制刷新 2026-07-24 `@TGB湖南人` 官方原文与原始图片证据。
- 脚本只输出公开文章和原图元数据；明确不调用 OCR、Qwen 或视觉识别，不写正式 TGB 行、不重折综合主因库、不重启服务。
- 若云端已有当天 raw evidence，脚本先备份旧目录；成功后追加两份云端运维日志。

Files:
- `ops/production/requests/2026-07-24-tgb-hunan-raw-evidence.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 北京时间目标日为 2026-07-24（周五）；任务分支从最新 `origin/main` 建立。
- 脚本固定校验目标日、官方文章域名、标题、manifest 日期/状态和至少一张原图下载成功；脚本保持纯 ASCII。
...
## 2026-07-24 - Codex - 修复预判回看次高、3日补取与细分题材命中

Changed:
- 日 K 进程缓存和磁盘缓存增加 `requiredThroughDay` 覆盖校验；同日保存但尚未包含目标交易日的数据不再挡住后续补取，null/不足数据也不会永久缓存。
- 预判回看为每只明星和龙头显式要求日 K 覆盖到所需的最晚后续交易日；每日收盘价库仍为首选，精确日 K 只在对应日期缺项时补齐次收与 3 日收盘。
- 细分题材家族键改为幂等处理：已有 `theme:`/`group:` 前缀时不再重复添加，修复“预判电网设备、盘后第一也是电网设备却显示未命中”。
- 未发生的未来交易日继续保持 null，不提前生成或猜测“次高/3日”数据。

Files:
- `kpl-stats-server.js`
- `tests/mainline-review.test.js`
- `tests/kline-review-freshness.test.js`
- `docs/DAILY_HANDOFF.md`

Validated:
- `node --check kpl-stats-server.js`、`git diff --check` 通过。
- 新增真实故障形状回归：北京时间同日保存但最后 K 线仍为前一交易日时强制重取；返回数据尚未覆盖目标日时下一次继续重试。
- 回看测试验证收盘价库缺项可由精确日期日 K 补齐，且请求明确覆盖第三个后续交易日。
- 全仓 `61/61` 个 `tests/*.test.js` 通过。
- 生产公开源人工核验：中国西电 2026-07-23 收盘 `13.65`、2026-07-24 最高 `14.48`，正确次高为 `+6.08%`。

Deployment:
- 本条提交时尚未部署生产；未修改云端文件、运行时数据库或缓存，未重启任何服务。
...
## 2026-07-25 - Codex - 修复历史主线结论被过早空快照覆盖

Changed:
- 定位 2026-07-23 “预判回看已有明星确认和主线，但今日结论显示无主线”的根因：15:01 的历史页面请求在收盘数据与 L2 归档尚未稳定时，把空结果永久冻结为 `api-after-close` 快照；同日 15:00 预测档案已正确保存电网设备、中国西电明星确认及两名龙头。
- 收盘后 15:00–15:30 改为归档缓冲期：页面访问可触发临时重算，但不得生成永久收盘快照；15:30 起才允许正式冻结。
- 历史接口遇到过早冻结的空快照时，只读恢复同日预测档案里已经通过正式 L2 正证据门槛的主线、明星与龙头；不改写原快照，不恢复弱候选，不补造预测档案未保存的板块涨幅、资金和盘面指标。
- 策略页给该类恢复卡增加“盘中预测档案校正”说明，并隐藏缺失的盘面指标和 L2 比值，避免以 `0` 或 `--` 冒充真实历史数据。
- 新增 2026-07-23 形态回归测试，并覆盖弱候选拒绝恢复、完整旧快照不被覆盖、15:30 冻结边界及前端不伪造指标。
...
Validated:
- 公网修复前基线已复核：`/api/strategy-mainlines?day=2026-07-23` 返回空主线，而 `/api/strategy-mainline-review?days=20` 返回电网设备、中国西电明星确认、立新能源与长缆科技两名龙头。
- 专项历史结论、QI 状态、三要件、策略 UI 测试通过。
- 全仓 `63/63` 个 `tests/*.test.js` 文件通过；`node --check kpl-stats-server.js` 与 `git diff --check` 通过。

Deployment:
- 尚未部署生产；合并后需同时发布后端与策略页 HTML，并重启主 Node 服务。
...
- Playwright 直接加载仓库 CSS 渲染三态卡片:确认态为白 Q/i + 蓝高亮点无外圈、
  预期态灰蓝、无明星态隐藏;1280 卡宽 600、390 卡宽 358,`bodyW` 无横向溢出。
- **更正**:首版我在这里写了"三态文字起始线一致",但只核对了外框都是 40×40,
  没有实测行内容的 `left`。Codex 复审实测为 164/164/166,并未对齐。已在下条修订。

### 按 Codex 复审修订两项 P2(同一 PR 内收口)

- **P2-1 三态文字起始线并未真正对齐** — 外框确实都是 40×40,但明星行自身左边框三态不同:
  确认/预期为 1px,无明星为 3px 状态边;边框计入盒宽,把无明星行内容整体右推 2px。
  保留 3px 状态边的视觉厚度,回扣该态 `padding-left: 11 → 9` 补偿。
  对齐约束由"外框等宽"改写为 **"`border-left` + `padding-left` 三态之和相等"**(当前均为 12),
  测试解析三条规则的实际数值并断言相等;已做变异验证:改回 11px 立即失败。
  Playwright 复测三态 `headLeft` 均为 166。
- **P2-2 实现注释与新规范冲突** — `kpl-dashboard_17_apple.html` 里仍写
  "徽章外框在三态下尺寸一致(确认态才显示金环)",会诱导后续 agent 把金环加回来。
  改为描述外框的占位职责并指向规范文档;新增断言禁止旧注释复现。
...
### #287 按 Local Claude 复审修订

- 第六条(二轮重放)的出处校正:原文写"fixture 错误跨轮存活",经当事评审方指正,
  该错误在其二轮内部、发布前自查发现并更正,未跨轮存活。已改为准确表述——
  错误形状被前置闸门短路导致断言空转,重放的意义是让单方自查不再是唯一防线。
  教训:守则的说服力建立在出处真实,出处本身也要经得起被校对。
- 采纳第七条:**对方的"不适用/无调用方/延后"声明必须独立验证,引用的测试要亲自读**。
  出处双重坐实:#284 的延后理由"已有测试证明无调用方"实测只锁了两个 builder 之一;
  #286 的删除 PR 又被查出漏删同类死链(WinRT OCR 子树)。
- 追加(同日):按 Local Claude 二次复核的增量建议,第一条矩阵式验证补"删除完整性"维度——
  删除类改动除验证"删的是否安全"外,必须回扫"同类是否还有未删的"(全量符号回扫/同类实现检索)。
  出处:#286 两位评审的第一反应都只验证了删除的安全性,WinRT 残留是靠全量回扫才浮出。
- 追加(同日):删除完整性检查升级为"孤儿函数集合差分"(base 与分支各扫一次
  仅有定义、零调用的函数,取差集)。该方法同时覆盖"同类残留未删"与"删除致幸存函数
  孤儿化"两种漏网;单纯回扫被删符号两者都查不到。出处:#286 的 WinRT 残留靠全量
  回扫浮出,而 WinRT 删除又孤儿化了 hashPart,后者只有集合差分能抓到(Local Claude
  的方法,Remote Claude 独立复刻数字一致:46→40,消除 7、新增 1)。
- 追加(同日):按 Local Claude 三次复核对齐第四条出处精度。原句"被当作措辞问题"不够准:
  同一底层变更,一方止步于观察(记为边界措辞修正 + 部署提示,最坏组合完全没看到),
  另一方把组合推演跑完才定红。改为如实写明"差别在推演这一步,不在观察"。
...
- 未部署。需发布 `kpl-stats-server.js` + `kpl-dashboard_17_apple.html` +
  ...
  先 CSS → HTML → 重启服务顺序;发布后核对线上引用 `?v=20260726d`(Local #292 指正:此行曾滞留 a 版)、
...
- 第二步(双源一致性条件命中率)与第三步(领先时长)见 2026-07-26 会话记录的策略页规划,
  Owner 暂只批了第一步。
- 追加(同日):Owner 反馈命中率行"看不清"——首版 11px + muted 全灰确实过弱。改为
  标签中性 12px、**数字主角 13.5px 亮色(--st-text)加粗**,边框升为 line-strong;
  仍不用金/红(本页确认态语义色)。CSS 版本 20260726a → 20260726b。
- 追加(同日,Owner 二次修订):命中率从列头下方独立徽标改为**直接进标题行**,
  跟在"东财主线预测/同花顺主线预测"后面同行展示(`命中 top1 X% · top3 Y%`,
  样本窗口与分母入 title 提示);单列兼容路径同样进"今日主线榜"标题行。
  文案压缩(去"近10日 N样本"前缀),零样本态为行内"近10日暂无样本"。
  新增断言锁定行内位置。CSS 版本 20260726b → 20260726c。
- 追加(同日,Codex #292 复审 P1 修正):历史日期页面不再附加命中率——回看统计固定以
  中国时区今天为锚,历史页挂当前口径会被误读成"截至该历史日的成绩"(日期穿越)。
  附加决策提取为 `attachPredictHitRatesIfToday` 纯函数:仅请求日==今天时附加,
  响应内新增 `asOfDay` 显式声明锚点;错误载荷与缓存未就绪原样返回。
  按复审要求补**行为测试**(真实调用,today/紧凑格式 today/历史/错误载荷/无缓存五分支),
  不再只做路由字符串断言;两处既有路由形式锁同步更新(意图不变)。
- 追加(同日,Local #292 复核修正):
  ① [阻断] ≤430px 窄屏折行回归——nowrap 徽标把同花顺列标题挤成两行(Local 量测页断点
     400→390;换真实 .ml-cols 单列规则复测为 405–420,阈值取 430)。修法:列头允许折行,
     标题 span(既有 flex 容器)独占整行、副标签退下一行;实测 340–430 全区间标题/副标签
     各 1 行、零溢出。加媒体查询测试锁。
  ② [必改] 部署校验行滞留 a 版——本条上方部署段的"核对线上引用"已更正为当前 20260726d。
  ③ 采纳三条建议:缓存刷新失败加 console.warn(可信度指标静默消失是最不该有的失效方式)、
     前端零样本判断直连 rate null 兜底、移除前端未用的 starWinRate/starTotal 字段。
  CSS 版本 20260726c → 20260726d。
- 追加(同日,Local #292 二轮新发现):asOfDay 跨午夜窗口错锚——缓存值 D 日 23:58 算出、
  D+1 00:05 命中 TTL 时会被现盖成 D+1。修法:缓存带锚点日 day,锚点在刷新触发时记录
  (计算跨午夜时记录的仍是数据真实锚点),锚点日≠今天即视为过期且绝不返回旧锚值。
  三条测试锁。Local 标注非阻断,但本 PR 主题即诚实,锚点字段不实十分钟也不留。
  另:Local 的"窄屏 flex-wrap 无条件生效"设计观察(换取布局不随数据可用性跳动/无 CLS)
  是有意识的取舍,勿当副作用去修——已由其复审记录在案。
- 追加(同日,Codex #292 三审 P1):跨午夜修复引入的退避回归——失败分支只更新 at 不更新
  day,首启(day='')/跨日失败后 day≠今天恒判过期,每次轮询重触发完整回看计算。
...
- 一来源一交易日最多一个样本；多只预期明星后来封板时取最早完成真实确认的一只，窗口汇总取中位数，
  不按股票数放大单日权重；时长只累计 `09:30–11:30`、`13:00–15:00` 可交易分钟。
- 盘后补确认的 `confirmedAt` 可能是涨停库保存时点，明确禁止用于时长；旧记录只有最终快照、
  没有首次事件轨迹时保持无样本。
- 预期信号在同股首次封板后才出现时不混入领先中位数，单独累计
  `mainlineLeadAfterFirstLimitSamples` 并在前端提示中披露，避免指标只报有利样本。
...
- 7月21日东财半导体为 top1 命中样本：兆易创新首次预期为北京时间 `11:04:29`，
  真实首次封板为 `14:08:05`；自然间隔 `183.6` 分钟，剔除午休后可交易领先
  `93.6` 分钟；涨停库 `savedAt=15:30` 未被误用。
- Local Claude 首轮复核发现自然时间包含午休会造成样本排序倒挂；修订后测试覆盖上午、
  下午、跨午休、开盘前一字板及封板后识别五种边界。云端近30日共 `2526` 条
  `firstLimitTime` 均为5/6位，三位格式风险未在生产数据出现。
...
## 2026-07-27 - Codex - 太极实业与雅克科技两月主因审计

Changed:
- 对生产主因库 2026-05-27 至 2026-07-27 做只读逐日审计，确认本次错挂不是数据源
  模糊或新股缺证据，而是策略明星挂载阶段忽略了已有主因约束。
- 太极实业近两月共有 7 个涨停记录日：
  06-18、06-24、06-25、07-01、07-09、07-21、07-27；最终归因全部为半导体或
  存储芯片。
- 雅克科技近两月共有 6 个涨停记录日：
  06-22、06-24、06-29、07-09、07-21、07-27；最终归因全部为半导体或存储芯片。
- 四源细节持续指向 DRAM/HBM、存储、先进封装、前驱体、电子气体和光刻胶；太极
  个别细节里出现“光伏”只属于并列业务标签，从未成为综合最终主因；雅克没有任何
  交易日被四源综合归为电力或电网设备。
...
Validated:
- 逐日读取生产 `/api/limit-up-main-reason-db/day`，再用
  `/api/limit-up-main-reason-db/stock` 对每个命中日复核四源明细。
- 审计共覆盖两只股票 13 条正式涨停主因记录；结论与 PR #304 修复后的证据优先级
  一致。

Deployment:
- 本次仅做只读生产审计和 Git 交接记录；未部署文件，未重启服务，未修改运行时数据。

Notes for next agent:
- 这两个案例应长期保留为明星归属负向回归基准：静态概念成员关系不得压过连续多日、
  多源一致的半导体/存储主因。
- 主因库不能只用于涨停次数和龙头评分；在明星确认挂载到板块之前必须执行同一套家族
  归属校验，否则会再次出现“基础数据正确、策略结论错误”。

## 2026-07-27 - Codex - 准备强刷当日 TGB 湖南人原始证据

Changed:
- 新增受保护、日期绑定的一次性生产脚本，用于强制刷新 2026-07-27 `@TGB湖南人`
  官方原文与原始图片证据。
- 脚本只输出公开文章和原图元数据；明确不调用 OCR、Qwen 或视觉识别，不写正式 TGB
  行、不重折综合主因库、不重启服务。

Files:
- `ops/production/requests/2026-07-27-tgb-hunan-raw-evidence.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 北京时间目标日为 2026-07-27（周一），仓库休市表不包含该日；公开
  `source-view` 已有当日 111 只综合、复盘啦和选股宝记录，按交易日处理。
...
## 2026-07-31 - Codex - 当日 TGB 湖南人原图字段截断阻断

Changed:
- PR #345 合入并通过受保护生产运行 `30627734881` 强制刷新 2026-07-31 官方原文
  和 14 张原始图片证据；正式 TGB、综合主因和服务进程均未改变。
- Codex 人工逐张排除头像、正文行情/统计图和回帖图，确认唯一正式候选为官方白底
  表格 `image-01-06.png`；标题、日期与 `@TGB湖南人` 水印均匹配。
- Codex 从该原图逐题材块、逐行、逐字段完成 98 行第一遍转录并回看原图完成第二遍
  人工复核；没有使用 OCR、Qwen 或其他自动视觉结果。
- 第二遍确认原图右缘截断两只股票的细分原因，按 SOP 停止正式写入和综合重折；新增
  日期绑定的只记日志生产请求，在重新确认无正式文件后记录阻断。

Files:
- `ops/production/requests/2026-07-31-tgb-hunan-blocked-log.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 文章：`https://www.tgb.cn/a/2tSJcjNYab7`，标题
  `7.31湖南人涨停复盘+晚间消息汇总`。
- 官方原图：`image-01-06.png`，1505003 字节，SHA-256
  `3048e335c7741d45f7231a5d44227006834b9a40bf99d85aa3ed60ff518726a3`；
  `_760w` 与 `_max` 官方 URL 返回同一份 530px 宽字节，放大不能恢复缺失像素。
- 人工候选预对账为 98 行、98 个唯一代码，`missingCodes=[]`、`extraCodes=[]`、
  无重复、`weakCount=0`；代码名称仅 `000032` 存在来源半角 `A` 与终盘全角 `Ａ`
  的明确规范化差异。
- 人工题材块实际行数：算力+数据中心 18、人工智能大模型 17、机器人 14、
  国产芯片 7、大消费 6、房地产 4、智能电网 4、PCB板 3、光通信 3、资产重组 3、
  其他热点 12、其他个股 7，合计 98。
- 阻断字段：`605178 时空科技` 的可见细分原因在“拟并购嘉合劲威（存储芯片”后
  被原图右缘截断；`605198 安德利` 在“拟收购宁波甬强科技（覆铜”后被截断。
- 公开 `source-view` 仍为淘股吧 0；既有综合归纳、复盘啦、选股宝和韭研均为 98，
  `sourceErrors=[]`。
- 受保护阻断日志运行 `30628998925` 重新读取生产终盘池：原始 99 行，过滤后 98 行、
  98 个唯一代码；代码集合 SHA-256 为
  `908743e0babcb01990c299e4b3af97774d988811f24982b382e28eff814d678f`，与人工候选
  一致，且正式 TGB 文件仍不存在。
- 首次日志运行 `30628781787` 因 Windows PowerShell 5.1 把无 BOM 脚本中的中文
  字符串按 ANSI 解析而在语法校验阶段失败；PR #347 将生产脚本改为纯 ASCII、标题
  以 Unicode 码点校验后复跑成功。失败发生在执行脚本前，没有追加日志或改动数据。

Deployment:
- 云端原始证据已刷新，运行 `30627734881` 已更新两份云端运维日志。
- 运行 `30628998925` 已把字段截断阻断追加到两份云端运维日志；幂等检查显示此前
  不存在同名记录。
- 正式 `tgb-hunan-structured/2026-07-31.json` 未写入；综合主因未重折。
- 没有部署应用代码，没有重启主站、Caddy、娱乐服务、SSH 或公司端 L2 worker。

Notes for next agent:
- 未获得完整可辨认的同一官方白底原图前，不得使用回帖图、其他来源或行情知识补齐
  两个截断字段，也不得把本次未通过闸的临时候选写入正式库。

## 2026-07-31 - Codex - 准备强刷当日 TGB 湖南人原始证据

Changed:
- 新增受保护、日期绑定的一次性生产脚本，用于强制刷新 2026-07-31 `@TGB湖南人`
  官方原文与原始图片证据。
- 脚本只输出公开文章和原图元数据；明确不调用 OCR、Qwen 或视觉识别，不写正式 TGB
  行、不重折综合主因库、不重启服务。

Files:
- `ops/production/requests/2026-07-31-tgb-hunan-raw-evidence.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 北京时间目标日为 2026-07-31（周五）；公开 `source-view` 已有当日复盘啦、选股宝
  和韭研各 98 条正式记录，按交易日处理。
- 脚本固定校验目标日、官方 `tgb.cn/a/` 链接、标题含“湖南人涨停复盘”、
  `raw-evidence-saved` 状态和至少一张成功下载的图片。
...
## 2026-07-31 - Codex - 修正当日短剧游戏策略结果

Changed:
- 使用已固定 SHA-256 的同日策略响应、同花顺终盘板块数据和公司 L2 任务证据，
  修正云端 2026-07-31 已冻结策略结果；未猜造或回算缺失字段。
- 恢复同花顺“短剧游戏”为预备主线：终盘 9 只涨停、板块涨幅 8.15%，保留
  `no-qualified-leader`，未擅自升级为正式主线。
- 将蓝色光标 `300058` 还原为 10:30 预期明星、10:45 封板确认，并从算力明星
  位置移除；删除由短视频个股错误生成的“消费”预备项。
- 修复前先备份原始预测档和冻结快照；云端两份操作日志均已追加记录。

Files:
- `docs/DAILY_HANDOFF.md`

Validated:
- 修复前云端预测档与冻结快照 SHA-256 分别为
  `0bf7e7299636ed495bcf82d422dc48f991b38eface45fcee569a011545671f35`、
  `b8f5cd0909dc5d59b2a1b51b8cfc44f0ff34e9db87179a145f53659a712d1da2`，与审计
  副本一致。
- 修复后公网 `/api/strategy-mainlines?day=2026-07-31` 返回 2 条正式主线、1 条
  “短剧游戏”预备主线；蓝色光标不再出现在算力明星列表。
- 修复后预测档与冻结快照 SHA-256 分别为
  `f15ba1c130e3275046f5f0501eeca40720b0805e8ec7e7225a3b330da13d1685`、
  `6c959b532abec122f2c510e3dd9f7f5b4cb55675cacb712efe6e014628fa664b`。

Deployment:
- 已直接修正云端运行时数据；未部署代码，未重启服务。
- 回退备份位于
  `C:\PandaDashboard\backups\strategy-result-repair-20260731`。

Notes for next agent:
- 本次仅修正 2026-07-31 的历史/当日结果；PR #342 已负责以后相同短视频板块的
  候选发现、家族归并和 L2 扫描覆盖。
- 运行时证据 JSON 未进入 Git。后续审计必须以交接中的 SHA-256 和云端备份为准，
  不得覆盖该备份或把“短剧游戏”从预备层强行提升为正式主线。

## 2026-07-31 - Codex - 修复明星股跨板块状态延迟

Changed:
- 修复候选股已经在其他板块得到 L2 预期/封板确认、却必须等待目标主线再次扫描后才
  显示的延迟。目标主线现在可复用该候选股当日最新的股票级 L2 状态，先显示
  “预期明星”，封板达标后即时升级“明星确认”。
- 板块扫描完成度仍只认同板或同主线家族任务；跨板块证据不能把未扫描主线伪装成
  已覆盖。候选股仍须属于目标主线成分池，并通过现有主因归属冲突校验。
- 若候选股后来开板或不再达标，最新股票级观察会降级/撤销较早的预期或当前确认；
  普通 `active` 资金活跃状态不跨板传播，防止通用板活跃股扩散到无关题材卡。
...
Validated:
- 生产只读证据：2026-07-31、股票 `300058`（蓝色光标）。10:30 的“快手概念”
  扫描在涨停前已满足预期明星条件，10:33 的“小红书概念”扫描继续确认预期状态；
  10:45 的“深股通”扫描在封板时满足明星确认。旧逻辑没有把前两者归入统一的
  短剧游戏家族，且宽泛的“AI应用”主因把它错误吸收到算力 AI。
- 安全摘录 SHA-256：
  `3ed428948b51bc8bf4606d7f55d2a82b4ac8e4f6a65401ac961b1bb41eda83d6`；
  原始 L2 运行时证据未进入 Git。
- 新增生产形状回归：其他板块的预期明星和封板确认可按最新状态挂回候选股，但不改变
  目标板块覆盖率；后续开板/失去明星条件会降级或撤销旧状态。
- `node --check kpl-stats-server.js`
- `node tests/star-l2-layers.test.js`
...
- 复审重点：确认股票级复用只适用于目标主线候选成分股，不绕过主因归属校验；
  通用板块任务不能提高目标主线扫描完成度；最新预期/确认/开板状态必须依时序替换，
  且普通 `active` 不得跨板传播。
- 本次不修改明星金额、比值、涨停或主线成立门槛。

## 2026-07-31 - Codex - 保留当日明星候选并修正蓝色光标短剧链
...
Validated:
- 生产只读证据显示蓝色光标不是 worker 漏扫：10:30 “快手概念”与 10:33
  “小红书概念”已在涨停前识别为预期明星；10:45 “深股通”任务盘中确认。
- 同花顺 AI视频使用双标识：内部概念 `plateId=309118`，页面指数代码
  `thsPlateCode=886068`。官方当日涨幅榜与项目持久化成分均不含蓝色光标，因而没有
  伪造它是该单板直接成分；蓝色光标通过快手/小红书同族证据归入短剧游戏。
  当天同花顺涨停原因
  `AI应用+出海广告+一季报增长` 以更具体的“出海广告”归入短剧游戏。
...
Changed:
- 修正 L2 扫描汇总把“最后一次扫描时仍封板”误写成“当前确认”的口径漏洞。
- 管理员 L2 历史接口增加可靠终盘涨停代码集；终盘数据形成后，“当前确认”只保留
  盘中达到明星确认且最终仍在涨停池的股票，“盘中曾确认”继续保留历史事实。
- 页面按事实依据动态显示“最近扫描确认”或“终盘确认”，不再把尚未复验的扫描结果
  统称为“当前”；悬停说明同步标明依据。
...
Validated:
- 生产事实抽查：2026-07-30 终盘综合来源 51 只涨停股中不含永鼎股份与利欧股份；
  原页面的“当前确认 1”来自永鼎股份封板时的最后一次 L2 扫描，不是终盘事实。
- 回归样本覆盖：盘中曾确认后炸板 → 当前确认 0、盘中曾确认保留；盘中确认且终盘仍
  封板 → 当前确认保留。

Deployment:
- 本条提交时仅为 GitHub 代码与测试变更；生产未部署，服务未重启。

Notes for next agent:
- 不要把“盘中曾确认”清零，它是复盘证据；只让“当前确认”受可靠终盘涨停池约束。
- 本次不改变明星阈值、主线成立条件、L2 worker、历史任务或正式策略排序。
...
Changed:
- PR #331 合入并通过受保护运行 `30539330910` 强制刷新官方原文和 16 张原始图片。
- PR #333 合入日期绑定人工写入请求，PR #334 根据生产写前闸补齐原始终盘池的
  北交所显式排除；受保护运行 `30540901627` 写入 Codex 从官方原图逐块、逐行、
  逐字段人工转录并二次人工复核的 51 行 TGB 正式来源，随后重折 2026-07-30
  综合主因库并完成公开接口复核。
- 生产脚本已把成功操作结果写入云端两份运维日志；没有部署应用代码到运行目录，
  也没有重启主站、Caddy、娱乐服务、SSH 或公司端 L2 worker。
...
Validated:
- 文章：`https://www.tgb.cn/a/2tR2UnZgD9B`；官方原图：
  `image-01-06.png`，585191 字节，SHA-256
  `7c7f27040c109b4ce46b6595febb92e78184351ec68e428f4f32decf4420f59f`。
- 正式图标题为“【7.30星期四】湖南人涨停复盘”，日期、白底表格和
  `@TGB湖南人` 水印均匹配；其他图表、回帖表、头像和小图均未进入正式行。
- “市场连板股”10 行为重复摘要块，未另计；“涨停炸板”19 行全部排除。
- 生产原始终盘池 52 行，显式排除北交所 `920685 新芝生物` 后正式基准为 51；
  正式写前、写后质量闸均为 51 行、51 个唯一代码，
  `missingCodes=[]`、`extraCodes=[]`、无重复、`weakCount=0`、
  `nameMismatches=[]`。原图“特力A”与终盘池“特  力Ａ”仅为空格和全角字符差异，
  已记录为 `NFKC+remove-whitespace` 规范化匹配。
- 正式题材块计数为大消费 11、汽车产业链 8、公告 6、智能电网 4、AI应用 3、
  其他热点 12、其他个股 7，合计 51。
- 公开 `source-view` 返回综合归纳、复盘啦、选股宝、韭研、淘股吧各 51；
  淘股吧与综合代码集合缺失/多余均为空，覆盖率和主因覆盖率均为 100%，
  低置信 0，`sourceErrors=[]`。
- 公开主因 day 接口返回 `count=51`，四个正式来源的 `rowCount/stockCount` 均为
  51，覆盖率和主因覆盖率均为 100%，`sourceErrors=[]`；`/health` 返回
  `{"ok":true}`。
- 生产哈希：终盘池
  `7a608467468a456b68194d8fef7e864c3ff8b888cd0426afd5a5e1ae61c65de4`；
  正式 TGB
  `7aceab6ab6f19bae4a5b04c19143e6f123cbab709aef8a87282d956fd98874e6`；
  综合主因
  `9fc50aebd32ef845b93019763280dc9bd4d7dd9dff348383e1be9fffb4951466`。
- 运行 `30540412589`、`30540542513` 因 payload 封装错误在解压阶段停止；
  `30540682166` 因原始池口径未固定北交所排除而在写前质量闸停止。三次都早于
  备份和正式写入，远端临时文件均已清理。

Deployment:
- 云端运行时数据已更新并生效；综合主因已重折。
- 备份目录：
  `C:\PandaDashboard\backups\tgb-hunan-manual-20260730-20260730120253`。
- 服务重启：无。

Notes for next agent:
- 2026-07-30 TGB 正式文件是受保护人工来源；普通同步和 force 均不得覆盖。

## 2026-07-30 - Codex - 修正 TGB 写前原始池口径

Changed:
- 修正日期绑定写入脚本的终盘池双口径固定值：生产原始池为 52 行，其中北交所
  `920685 新芝生物` 按正式规则排除，非 ST、非北交所、非新股前缀合格池仍为
  人工转录的 51 行。

Files:
- `ops/production/requests/2026-07-30-tgb-hunan-write.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 受保护运行 `30540412589`、`30540542513` 因 payload 封装错误在解压阶段停止；
  运行 `30540682166` 在写前终盘池闸明确返回原始 52、合格 51、排除
  `920685 新芝生物`。三次均早于备份和正式写入，远端临时文件均已清理。
- 人工候选、payload 内容 SHA-256、51 行代码集合、题材块计数和原图证据均未改变。

Deployment:
- 正式 TGB、综合主因库和服务进程仍未改变；服务重启：无。

Notes for next agent:
- 后续受保护写入必须同时固定原始池 52 和合格池 51，且只允许排除
  `920685`；若生产池再次变化应继续停止，不得调整人工行凑数。

## 2026-07-30 - Codex - 准备写入当日 TGB 湖南人人工复盘

Changed:
- PR #331 已合入并通过受保护生产运行 `30539330910` 强制刷新当日官方原文和
  16 张原始图片证据；正式来源、综合主因和服务进程均未改变。
- Codex 已从官方 `image-01-06.png` 白底表格按题材块逐行人工转录 51 行，并回看
  原图逐字段完成第二遍人工复核；未使用 OCR、Qwen 或其他自动视觉结果。
- 新增日期绑定写入脚本和工作流 secret 白名单，正式行只通过加密生产环境 payload
  传递，不进入 Git；脚本在任何写入前校验原图哈希、终盘池、代码/名称、题材计数
  和固定来源字段，失败即停止。
...
Validated:
- 文章：`https://www.tgb.cn/a/2tR2UnZgD9B`，标题
  `7.30湖南人涨停复盘+晚间消息汇总`。
- 正式原图：`image-01-06.png`，585191 字节，SHA-256
  `7c7f27040c109b4ce46b6595febb92e78184351ec68e428f4f32decf4420f59f`；
  其他图表、回帖表、头像和小图均排除。
- “市场连板股”10 行为重复摘要块，不另计；“涨停炸板”19 行全部排除。正式题材块：
  大消费 11、汽车产业链 8、公告 6、智能电网 4、AI应用 3、其他热点 12、
  其他个股 7，合计 51。
- 人工候选与公开同日终盘口径预对账：51 行、51 个唯一代码，
  `missingCodes=[]`、`extraCodes=[]`、无重复、`weakCount=0`。原图“特力A”与
  终盘池“特  力Ａ”仅为空格和全角字符差异，已显式记录为
  `NFKC+remove-whitespace` 规范化匹配。

Deployment:
- 原始证据已刷新并写入云端两份运维日志；正式 TGB 写入和综合主因重折尚未执行。
- 服务重启：无。
...
Changed:
- PR #321 合入并通过受保护运行 `30355531306` 强制刷新官方原文和 20 张原始图片。
- PR #322 合入日期绑定人工写入请求；首次运行 `30356379645` 因原始终盘池包含
  北交所 `920690 捷众科技` 而在写前安全停止，没有修改运行时数据。
- PR #323 修正为同时校验 61 行原始池和排除北交所后的 60 行正式口径池；受保护运行
  `30356662366` 写入 Codex 人工双遍复核的 60 行 TGB 正式来源，随后重折
  2026-07-28 综合主因库并完成公开接口复核。
- 生产脚本已把成功操作结果写入云端两份运维日志；没有部署应用代码到运行目录，也
  没有重启主站、Caddy、娱乐服务、SSH 或公司端 L2 worker。
...
Validated:
- 文章：`https://www.tgb.cn/a/2tNGSu2WSSN`；官方原图：
  `image-01-06.png`，SHA-256
  `e328eeeb794dff1e1a32c1bbd50e3ccb453f70f1c6922508e28801d5ae7490bd`。
- 原始终盘池 61 行，显式排除北交所 `920690 捷众科技` 后正式基准 60 行；正式写前
  与写后质量闸一致：60 行、60 个唯一代码、`missingCodes=[]`、`extraCodes=[]`、
  无重复、`weakCount=0`、`nameMismatches=[]`，10 个题材块合计 60。
- 来源忠实名称差异已按固定代码显式记录：`000045` 原图“深纺织A”/终盘池
  “深纺织Ａ”，采用 `NFKC+remove-whitespace` 对账，不改写来源名称。
- 公开 `source-view` 与主因 day 接口均返回淘股吧 60、综合归纳 60、覆盖率 100%、
  主因覆盖率 100%、低置信 0、`sourceErrors=[]`；独立代码集合复核缺失/多余均为空。
- 复盘啦与选股宝各 60；韭研独立来源当时仍为 0，不影响本次 TGB 质量闸。
- `/health` 返回 `{"ok":true}`。
- 生产哈希：终盘池
  `2f77e79634e9a092dc53de6ab8cfaf86d6882ad2f0e86e26d9fe9cc2c7f738cb`；
  正式 TGB
  `47eb324a2d0882694130c36dc259c605016cf9c766c3d6757b328a86e0fefdb5`；
  综合主因
  `11f7e7e0b1d4e977817170865aab9cc3e7d0b2932b47b5e548ca54307a91fb8f`。

Deployment:
- 云端运行时数据已更新并生效；综合主因已重折。
- 备份目录：
  `C:\PandaDashboard\backups\tgb-hunan-manual-20260728-20260728115445`。
- 服务重启：无。

Notes for next agent:
- 2026-07-28 TGB 正式文件是受保护人工来源；普通同步和 force 均不得覆盖。
...
Changed:
- 受保护生产运行 `30262706867` 已强制刷新官方原文与 24 张原始图片；没有写正式行、
  没有重折综合主因库、没有重启服务。
- 人工筛选 `image-01-06.png`：标题、日期、白底题材表格和 `@TGB湖南人` 水印均匹配；
  排除同花顺红图、头像、二维码、回帖/消息图以及原图底部“涨停炸板”区。
- Codex 已按原图 15 个正式题材块逐行录入并完成第二遍逐字段人工复核，共 111 行；
  全程未使用 Qwen、OCR 或其他自动视觉结果生成、补全或校验正式行。
- 新增日期绑定写入脚本与加密 payload 接线；脚本在写前重读生产终盘池，要求 111 行、
  111 个唯一代码、`missingCodes=[]`、`extraCodes=[]`、`weakCount=0`，并固定题材块顺序
  和计数总和。通过后先备份、原子写正式 TGB、重折当日综合主因库、验证公开
  `source-view`/主因库/健康接口；任一步失败均回滚。

Files:
- `.github/workflows/production-ops.yml`
- `ops/production/requests/2026-07-27-tgb-hunan-write.ps1`
- `tests/tgb-20260727-production-request.test.js`
- `docs/DAILY_HANDOFF.md`

Validated:
- 文章：`https://www.tgb.cn/a/2tM2uLbdlIs`，标题：
  `7.27湖南人涨停复盘+晚间消息汇总`。
- 官方原图：`image-01-06.png`，1,045,427 字节，SHA-256
  `7ae39e7b6227e2eb98abbdfb92473aea0ed84cd559e9eb3623b0c4d2e8be773c`。
- 人工块计数：
  大消费 13、机器人 13、智能电网 11、PCB板 10、半导体 9、医药 9、被动元件 7、
  锂电产业链 7、脑机接口 6、光通信 4、算力 4、燃气轮机 3、业绩增长 3、公告 3、
  其他 9；合计 111。
- 写前本地代码对账：111 行、111 个唯一代码、无缺失、无多余、无重复、无空字段。
  来源忠实保留两处显式名称差异：原图“华润新能源”/终盘池“华润新能”，以及原图
  “七匹狼”/终盘池“七 匹 狼”；脚本仅按固定代码和明确别名匹配，不改写来源名称。
...
- 目录板块在同源匹配、已有至少 2 个当日信号股且资金可能过闸时补取成分股；
  对没有既有题材 seed、但东财超大单净流入已达 5 亿元的正涨 catalog 板，也允许
  独立补水，堵住“没有 seed 就永远拿不到 zt/无法派单”的首日方向循环。
- 盘中正式涨停底库尚未落盘时，复用同花顺实时涨停池的股票代码作为当日涨停事实；
  空 `reason_type` 只用于涨停成员交集，不参与主因归属。随后用真实成分股逐只回填
  `codes/zt`，并入原有双源统一派单池。
- 复审修复：实时涨停池的空主因行不得覆盖当天四源综合主因库的正式证据；独立目录
  发现明确拒绝东财旧 `f62` 主力净流入口径，只接受可追溯的 `f66` 超大单净流入。
...
- 全部 `tests/*.test.js`
- `git diff --check`
- 2026-07-28 生产只读证据：公司 worker 09:42 后连续完成 L2 任务，故不是 worker
  离线；东财 `BK0884 光刻机(胶)` 当时未产生任何任务，但实时 catalog 超大单净流入
  约 5.52 亿元，96 只成分股中有 9 只实时涨停。旧链路因其约排第 127、未进主通道
  前 5/补选前 3且 `zt=null` 而漏派单。
- 新回归覆盖 `BK0884` 等价场景：无既有 seed、资金 5.52 亿元、真实成分交集至少
  2 只涨停时可派单；低于资金门槛、成分缺失或涨停不足仍不得派单。
- 新回归锁定：空实时主因不能遮蔽四源综合主因，非空实时主因仍可作为当日最新证据；
  东财旧主力净流入口径不能触发独立目录发现。
- 2026-07-28 生产目录实测共 495 个板块，其中正涨且 `f66>=5亿` 的只有 6 个；
  暂不采用 top-K 截断，避免重新遗漏已经满足正式资金门槛的低排名板。若后续强势日
  候选规模明显扩大，应采用可轮转的请求预算，而不是永久丢弃排名靠后的合格板。
...
## 2026-07-28 - Codex - 准备强刷当日 TGB 湖南人原始证据

Changed:
- 新增受保护、日期绑定的一次性生产脚本，用于强制刷新 2026-07-28 `@TGB湖南人`
  官方原文与原始图片证据。
- 脚本只输出公开文章和原图元数据；明确不调用 OCR、Qwen 或视觉识别，不写正式 TGB
  行、不重折综合主因库、不重启服务。

Files:
- `ops/production/requests/2026-07-28-tgb-hunan-raw-evidence.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 北京时间目标日为 2026-07-28（周二）；公开 `source-view` 已有当日复盘啦和选股宝
  各 60 条正式记录，按交易日处理。
- 脚本固定校验目标日、官方 `tgb.cn/a/` 链接、标题含“湖南人涨停复盘”、
  `raw-evidence-saved` 状态和至少一张成功下载的图片。
...
## 2026-07-29 - Codex - 准备强刷当日 TGB 湖南人原始证据

Changed:
- 新增受保护、日期绑定的一次性生产脚本，用于强制刷新 2026-07-29 `@TGB湖南人`
  官方原文与原始图片证据。
- 脚本只输出公开文章和原图元数据；明确不调用 OCR、Qwen 或视觉识别，不写正式 TGB
  行、不重折综合主因库、不重启服务。

Files:
- `ops/production/requests/2026-07-29-tgb-hunan-raw-evidence.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 北京时间目标日为 2026-07-29（周三）；公开 `source-view` 已有当日复盘啦和选股宝
  各 81 条正式记录，按交易日处理。
- 脚本固定校验目标日、官方 `tgb.cn/a/` 链接、标题含“湖南人涨停复盘”、
  `raw-evidence-saved` 状态和至少一张成功下载的图片。
...
## 2026-07-29 - Codex - 准备写入当日 TGB 湖南人人工复盘

Changed:
- PR #325 已合入并通过受保护生产运行 `30448387725` 强制刷新当日官方原文和
  19 张原始图片证据；正式来源、综合主因和服务进程均未改变。
- Codex 已从官方 `image-01-06.png` 白底表格按题材块逐行人工转录 81 行，并回看
  原图逐字段完成第二遍人工复核；未使用 OCR、Qwen 或其他自动视觉结果。
- 新增日期绑定写入脚本和工作流 secret 白名单，正式行只通过加密生产环境 payload
  传递，不进入 Git；脚本在任何写入前校验原图哈希、终盘池、代码/名称、题材计数
  和固定来源字段，失败即停止。
...
Files:
- `.github/workflows/production-ops.yml`
- `ops/production/requests/2026-07-29-tgb-hunan-write.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 文章：`https://www.tgb.cn/a/2tPnCn1JGfJ`，标题
  `7.29湖南人涨停复盘+晚间消息汇总`。
- 正式原图：`image-01-06.png`，823290 字节，SHA-256
  `b54b3e3a80d8f97c25d019d34a184c4ad7fef76185bded81cb3efd6e614f1810`；
  同花顺红图 `image-01-15.png`、无官方水印回帖图 `image-01-11.png` 已明确排除。
- “市场连板股”11 行为重复摘要块，不另计；“涨停炸板”14 行全部排除。正式题材块：
  大消费 25、房地产 6、机器人 6、锂电池 6、智能电网 6、半导体 5、化工 4、
  大金融 3、教育 3、游戏 3、其他热点 8、其他个股 6，合计 81。
- 人工候选与公开同日终盘口径预对账：81 行、81 个唯一代码，
  `missingCodes=[]`、`extraCodes=[]`、无重复、`weakCount=0`、无名称差异。
...
## 2026-07-29 - Codex - 当日 TGB 湖南人复盘入库完成

Changed:
- PR #325 合入并通过受保护运行 `30448387725` 强制刷新官方原文和 19 张原始图片。
- PR #326 合入日期绑定人工写入请求；受保护运行 `30449509475` 写入 Codex 从官方
  原图逐块、逐行、逐字段人工转录并二次人工复核的 81 行 TGB 正式来源，随后重折
  2026-07-29 综合主因库并完成公开接口复核。
- 生产脚本已把成功操作结果写入云端两份运维日志；没有部署应用代码到运行目录，也
  没有重启主站、Caddy、娱乐服务、SSH 或公司端 L2 worker。
...
Validated:
- 文章：`https://www.tgb.cn/a/2tPnCn1JGfJ`；官方原图：
  `image-01-06.png`，823290 字节，SHA-256
  `b54b3e3a80d8f97c25d019d34a184c4ad7fef76185bded81cb3efd6e614f1810`。
- 正式图标题、日期、白底表格和 `@TGB湖南人` 水印均匹配；同花顺红图
  `image-01-15.png`、无官方水印回帖表格 `image-01-11.png`、头像、广告和炸板区
  均未进入正式行。
- 生产终盘池与正式写前、写后质量闸一致：81 行、81 个唯一代码，无 ST、北交所或
  新股前缀排除项；`missingCodes=[]`、`extraCodes=[]`、无重复、`weakCount=0`、
  `nameMismatches=[]`、`nameDifferences=[]`。
- 正式题材块计数为大消费 25、房地产 6、机器人 6、锂电池 6、智能电网 6、
  半导体 5、化工 4、大金融 3、教育 3、游戏 3、其他热点 8、其他个股 6，合计 81。
- “市场连板股”11 行为重复摘要块，未另计；“涨停炸板”14 行全部排除。
- 公开 `source-view` 与主因 day 接口均返回淘股吧 81、综合归纳 81、覆盖率 100%、
  主因覆盖率 100%、低置信 0、`sourceErrors=[]`；独立代码集合复核缺失/多余均为空。
- 复盘啦与选股宝各 81；韭研独立来源当时仍为 0，不影响本次 TGB 质量闸。
- `/health` 返回 `{"ok":true}`。
- 生产哈希：终盘池
  `eb348211478abdbdcd0a2c28c1f3254f4a876b3afdc94abf79ac721a32ada658`；
  正式 TGB
  `52fb299701dab49952a456f37cac5843a9553f257d0b142569a2c85d52d45ba5`；
  综合主因
  `c656e0a8f1713044e017e3eb6b959dbc3ea3f3491bbc05abe43649c2527653d2`。

Deployment:
- 云端运行时数据已更新并生效；综合主因已重折。
- 备份目录：
  `C:\PandaDashboard\backups\tgb-hunan-manual-20260729-20260729115713`。
- 服务重启：无。
...
## 2026-07-31 - Codex - 部署双源短视频主线发现修复

Changed:
- PR #342 已合并到 `main`。策略把 AI视频、快手、小红书、短剧互动游戏、文生视频
  与出海广告统一归入 `theme:短剧游戏`，但东财和同花顺仍独立计算，不合并资金或
  跨源复用明星证据。
- 已通过既有 L2 硬门槛的同日强板不再被前 5 主通道和 3 个补选名额截断；跨日和
  缺内部日期的快照继续拒绝。候选换榜后仅保留当天真实出现过的预期/确认明星轨迹。
- 蓝色光标同类股票可在快手/小红书扫描阶段先成为预期明星，封板后再升级明星确认；
  具体“出海广告”证据优先于宽泛“AI应用”，不得误归算力 AI。
...
Validated:
- 两份独立复审完成；远端 Claude 明确 approved，本地 Claude 未发现代码阻断项。
- GitHub Actions production run `30620783032` 成功；公网 `/health` 返回
  `{"ok":true}`。
- 上线前后使用同一 `/api/strategy-mainline-review?days=15` 参数对照：主线成立率
  66.7%（4/6）、预期明星封板率 25%（1/4）、领先时长中位数 93.6 分钟（1 个
  样本）、预备明星封板率 0%（0/14）均未变化，词典归并没有改变当前 15 日回看
  统计。上线前响应 SHA-256 为
  `1315b9575212ad96549e10a934c54a074cac423a116f6bed0fe11dd912f48d82`，上线后为
  `5bc274c222750cfe3df2ebe14a794397c7bce23ef3151f461ea05e874b5282d3`；响应含动态
  时间字段，故整包哈希不同，核心统计逐项一致。
- 生产部署提交为 `829d8e2a8c9b8422c31c4ef7f826b0dc0a5ebb55`，发布清单固定为两份已审文件。

Deployment:
- 已部署并生效；仅重启 `Panda Dashboard Server`，未重启 Caddy、娱乐服务、SSH
  或公司端 L2 worker。
- 回退备份：
  `C:\PandaDashboard\_deploy-backups\github-30620783032-1`。
- 受保护部署器已自动追加云端两份操作日志。

Notes for next agent:
- 2026-07-31 已冻结的旧预测档保持当时实际展示结果，不通过新词典猜造历史候选；
  本次修复面向后续同类盘面。若需要恢复 07-31，只能使用已保存且 SHA 固定的真实
  盘中预测片段并先备份。
- 首个交易日观察 `scanSupplement.hardQualified` 的实际达标板数量、派单等待与
  `coverage-insufficient`。不得为了控制数量重新加入会漏掉已过硬门槛强板的静态
  topN；只有出现真实性能压力时才讨论公平轮转预算。

## 2026-07-30 - Codex - L2 自动扫描家族公平调度

Changed:
- 根据 2026-07-29 生产任务证据修正自动派发顺序：确认扫描与错误重试固定优先；
  普通名额在首次发现与增强复扫之间轮换，冷启动先发现，避免已扫板块持续增强时
  压住新合格板块，也避免无限目录候选反过来饿死真实增强复扫。
...
- 新增回归反例：20 亿元高流入板的增强复扫不得压住 6 亿元首次合格板；同一家族
  已有板块覆盖时，不得凭更高资金继续抢占零覆盖家族的首次机会；首次发现获得一个
  普通名额后，下一普通名额会回到真实增强复扫。
- `node --check kpl-stats-server.js`
...
## 2026-08-02 - Codex - 重试 7.31 TGB 官方原图仍阻断

Changed:
- 按用户要求重试 2026-07-31 TGB 湖南人复盘；北京时间 2026-08-02 为周日，未把
  非交易日误当成新的目标日。
- 复用已合入 `main` 的日期绑定 raw-evidence 请求，通过受保护运行 `30752232728`
  再次强制刷新官方文章和图片，并在刷新前备份旧 raw evidence。
- 新增纯 ASCII、log-only 的重试阻断请求；PR #349 合入后，受保护运行
  `30752408623` 验证生产证据、终盘池和正式文件状态，并把“重试后仍阻断”写入
  两份云端运维日志。

Files:
- `ops/production/requests/2026-08-02-tgb-hunan-20260731-retry-blocked-log.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 文章仍为 `https://www.tgb.cn/a/2tSJcjNYab7`，标题
  `7.31湖南人涨停复盘+晚间消息汇总`。
- 刷新后共保存 22 张图片；新增的 8 张均为约 60×60 的头像类小图，正式白底候选
  仍只有 `image-01-06.png`。
- `image-01-06.png` 仍为 530×8162、1505003 字节，SHA-256 仍为
  `3048e335c7741d45f7231a5d44227006834b9a40bf99d85aa3ed60ff518726a3`；官方
  `_760w` 与 `_max` URL 字节完全相同，作者/CDN 没有替换主图。
- `605178 时空科技` 与 `605198 安德利` 的细分原因仍在原图右缘被截断，无法从
  官方白底原图辨认完整字段；禁止用新增头像、回帖图、其他来源或常识补齐。
- 生产终盘池仍为原始 99 行、过滤后 98 行/98 个唯一代码，集合 SHA-256 仍为
  `908743e0babcb01990c299e4b3af97774d988811f24982b382e28eff814d678f`。
- 公开 `source-view` 仍为淘股吧 0，综合归纳、复盘啦、选股宝和韭研各 98，
  `sourceErrors=[]`；`/health` 返回 `{"ok":true}`。

Deployment:
- 云端 raw evidence 已刷新；旧证据备份为
  `C:\PandaDashboard\backups\tgb-hunan-raw-20260731-20260802-223042`。
- 两份云端运维日志已记录原始证据重刷和重试仍阻断。
- 正式 `tgb-hunan-structured/2026-07-31.json` 仍未写入；综合主因未重折。
- 没有部署应用代码，没有重启主站、Caddy、娱乐服务、SSH 或公司端 L2 worker。
...
- 保留来源标量 `ztReported`、逐股可见数 `ztObserved`、归属前有效数 `ztRaw`、修正后
  `zt`、合格代码和冲突明细；只修正策略派生统计，不改涨停底库、主因库或来源快照。
  主因未知者及来源标量中尚无法逐股识别的差额继续保留，避免错杀首日新题材。
- 归属修正提前到实时主线 seed 生成之前，防止硬件股虽然未通过 L2 扫描门槛，却仍残留在
  软件题材的 `todayCodes` 或 `countFallback` 中。
- 根据 Local Claude 阻断复审统一了扣减总体：来源标量小于逐股列表时，以逐股列表为归属前
  有效数量；来源标量大于可见列表时，未识别差额按未知保留并标 `partial`。因此不会再出现
  “合格代码仍有 2 只、修正后涨停数却为 0”的矛盾，也不会因覆盖不全过度扣减。
...
- 云端直接核对 2026-07-31 原始 `kpl-snapshots/5` 与 `/6`（非证据包过滤视图）：所有板块的
  `ztCount` 均与各自完整 `cardData[plateId].ztList` 唯一代码数一致。
- `git diff --check`
...
## 2026-08-03 - Codex - 电力父主线接纳发电细分明星

Changed:
- 修复“电力”父主线因主因细分为火电/绿电/水电而拒绝真实明星的问题：三个发电侧
  子题材现在可以支撑 `theme:电力`，但兄弟子题材之间仍不能互借明星。
- 兼容范围刻意不包含电网设备、特高压、核电工程、燃气轮机等设备/工程方向，避免重新
  引入太极实业、雅克科技一类静态成分错挂。
- 新增华电辽能真实口径回归：当日“火电”可进入电力父主线，不能进入绿色电力；历史
  “绿色电力”在盘中尚无当日主因时也可支撑电力父主线。
...
Validated:
- 生产只读核验：2026-08-03 电力候选为 5 只涨停、4 个板块证据，相关 L2 扫描已完成；
  华电辽能在绿色电力任务中达到明星确认数值，但当日同花顺主因为“火电”，被旧精确家族
  规则拒绝。
- 云端历史综合主因核验：华电辽能 2026-07-20、07-22 均为“电力/绿色电力”，
  2026-08-03 细分原因为“电力(绿色电力)”。
- `node --check kpl-stats-server.js`
- 明星归属、AI 软件家族、QI 主线状态、预期明星粘性、预判回看、历史结论、板级涨停
  回填、catalog L2 富化、三要件、双源主线、资金闸与复扫等 15 组专项测试通过。
- `git diff --check`

Deployment:
- GitHub only；尚未合并、未部署、未重启服务，未改任何生产数据库、预测档或 L2 任务。

Notes for next agent:
- 复核时重点确认兼容方向只能是“发电细分 → 电力父主线”：火电不能进入绿电，电网设备
  不能进入电力，半导体等无关主因继续保持冲突拒绝。
...
## 2026-08-28 - Codex - TGB 人工转录写前终盘池检查请求

Changed:
- 受保护 raw-evidence 运行 `33168057953` 已保存官方文章及23张原始图片；人工选定
  `image-01-06.png`，排除同花顺红图、回帖图、头像、小图、市场连板摘要和炸板区。
- Codex 已按官方原图完成81条正式题材行候选的第一遍逐字段人工录入；新增日期绑定的只读写前检查，
  读取生产终盘池并输出过滤后的代码/名称、排除项和代码集 SHA-256，不写任何文件。

Files:
- `ops/production/requests/2026-08-28-tgb-hunan-prewrite-inspect.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 官方文章 `https://www.tgb.cn/a/2uD6GIwPL0s`，标题 `8.28湖南人涨停复盘+晚间消息汇总`。
- 官方白底表格原图 `image-01-06.png`，530×4905、876211字节、SHA-256
  `8e2f1542914c01777d25014f5b3d77cc86b6434783637d8d335f23392b6eca74`。
- 第一遍题材计数：农业12、AI应用8、机器人5、算力5、医药5、大消费4、房地产4、国产芯片4、
  智能电网4、化工4、氟化工/PTFE3、其他热点12、其他个股11，合计81；市场连板股17行为摘要重复，
  涨停炸板16行排除。
...
Notes for next agent:
- 只读检查必须确认正式文件尚不存在、81条候选与过滤终盘池完全一致后，才可进行第二遍原图逐字段
  复核和受保护写入；任一 missing/extra/重复/弱字段或未记录名称差异都必须阻断。

## 2026-08-28 - Codex - TGB 湖南人81行正式写入请求

Changed:
- 受保护只读运行 `33168894814` 确认生产终盘原始池82/82，排除北交所 `920895` 后合格池81/81，
  正式 TGB 文件不存在；人工候选代码集与合格终盘池完全一致。
- 官方原图第二遍逐块、逐行、逐字段人工复核完成；新增加密日期绑定的81行正式写入请求，固定人工
...
- 终盘池代码集 SHA-256 `cc89679f67f77f1c38d62140b67f8205bdfd7d408eefa840ba2f6d22c5196b13`；
  正式写入前必须保持不变。
- 官方文章 `https://www.tgb.cn/a/2uD6GIwPL0s`；原图 `image-01-06.png`，876211字节，
  SHA-256 `8e2f1542914c01777d25014f5b3d77cc86b6434783637d8d335f23392b6eca74`。
...
- 只有本日期绑定脚本在 `main` 合并并经 `production` 环境批准后才可执行；公开淘股吧与综合归纳均须
  精确为81、覆盖率100%、低质量0、`sourceErrors=[]`，否则回滚并报告阻断。

## 2026-08-26 - Codex - TGB 人工转录写前终盘池检查请求

Changed:
- 受保护 raw-evidence 运行 `32964832189` 已保存官方文章及20张成功下载的原始图片；人工选定
  `image-01-06.png`，排除同花顺红图、统计/行情图、前一日其他作者表格、回帖图、头像、小图、
  市场连板摘要和涨停炸板区。
- Codex 已按官方原图完成52条正式题材行候选的第一遍逐字段人工录入；新增日期绑定的只读写前检查，
  读取生产终盘池并输出过滤后的代码/名称、排除项和代码集 SHA-256，不写任何文件。

Files:
- `ops/production/requests/2026-08-26-tgb-hunan-prewrite-inspect.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 官方文章 `https://www.tgb.cn/a/2uzSn8RLuWv`，标题 `8.26湖南人涨停复盘+晚间消息汇总`。
- 官方白底表格原图 `image-01-06.png`，530×3783、644390字节、SHA-256
  `32a6d6c2651fe43ed5eece818ae0ad4f0064f22aac58b47fe6f30997ae4725c8`。
- 第一遍题材计数：大金融6、光通信6、公告5、医药5、黄金4、半导体3、可控核聚变3、农业3、有色金属3、
  其他热点6、其他个股8，合计52；市场连板股13行为重复摘要，涨停炸板20行排除。

Deployment:
- 原始证据强刷已执行并更新两份云端日志；正式 TGB 行尚未写入，综合主因未重折，服务未重启。

Notes for next agent:
- 只读检查必须确认正式文件尚不存在、52条候选与过滤终盘池完全一致后，才可进行第二遍原图逐字段
  复核和受保护写入；任一 missing/extra/重复/弱字段或未记录名称差异都必须阻断。

## 2026-08-26 - Codex - TGB 湖南人52行正式写入请求

Changed:
- 受保护只读运行 `32966313434` 确认生产终盘原始池/合格池均52/52，无排除项，正式 TGB 文件
  不存在；人工候选代码集与合格终盘池完全一致。
- 官方原图第二遍逐块、逐行、逐字段人工复核完成；新增加密日期绑定的52行正式写入请求，固定人工
  payload、文章和原图哈希，重验题材计数、代码名称、missing/extra/重复/弱字段，备份后原子写入、
  重折并验证公网状态，任一写后闸失败自动回滚。

Files:
- `.github/workflows/production-ops.yml`
- `ops/production/requests/2026-08-26-tgb-hunan-write.ps1`
- `tests/tgb-20260826-production-request.test.js`
- `docs/DAILY_HANDOFF.md`

Validated:
- 人工 payload：52行/52唯一代码，11个题材块合计52，`missingCodes=[]`、`extraCodes=[]`、重复0、
  `weakCount=0`；唯一名称格式差异为 `000997` 新大陆/新 大 陆，NFKC/去空格一致；payload
  SHA-256 `89ac892e050510027c653802d221bdb6318f622de81ce8859be908c937b76a89`。
- 终盘池代码集 SHA-256 `7d442f5aef514eff8f7855f99298ccbb7993ae83a85d11beecf139f3ffb8b9ee`；
  正式写入前必须保持不变。
- 官方文章 `https://www.tgb.cn/a/2uzSn8RLuWv`；原图 `image-01-06.png`，644390字节，
  SHA-256 `32a6d6c2651fe43ed5eece818ae0ad4f0064f22aac58b47fe6f30997ae4725c8`。
...
Notes for next agent:
- 只有本日期绑定脚本在 `main` 合并并经 `production` 环境批准后才可执行；公开淘股吧与综合归纳均须
  精确为52、覆盖率100%、低质量0、`sourceErrors=[]`，否则回滚并报告阻断。

## 2026-08-21 - Codex - TGB 人工转录写前终盘池检查请求

Changed:
- 受保护 raw-evidence 运行 `32477993992` 已保存官方文章及19张原始图片；人工选定
  `image-01-06.png`，排除同花顺红图、回帖图、头像、小图、深色行情/统计图、市场连板摘要和炸板区。
- Codex 已按官方原图完成54条正式题材行候选的第一遍逐字段人工录入；新增日期绑定的只读写前检查，
  读取生产终盘池并输出过滤后的代码/名称、排除项和代码集 SHA-256，不写任何文件。
...
Validated:
- 官方文章 `https://www.tgb.cn/a/2uruhLpqJe9`，标题 `8.21湖南人涨停复盘+晚间消息汇总`。
- 官方白底表格原图 `image-01-06.png`，530×3699、621820字节、SHA-256
  `818b816889183ca8f8a2bf95eedf8c1c7f16e17b21872efcb66e4a669db02554`。
- 第一遍题材计数：机器人10、光通信9、医药8、半导体4、黄金4、大消费3、液冷3、其他热点4、
  其他个股9，合计54；市场连板股11行为重复摘要，涨停炸板18行排除。
...
Changed:
- 受保护 raw-evidence 运行 `32364722634` 已保存官方文章及18张原始图片；人工选定
  `image-01-06.png`，排除深色行情长图、回帖图、头像、小图、市场连板摘要和涨停炸板区。
- Codex 已按官方原图完成78条正式题材行候选的第一遍逐字段人工录入；新增日期绑定的只读写前检查，
  读取生产终盘池并输出过滤后的代码/名称、排除项和代码集 SHA-256，不写任何文件。

Files:
- `ops/production/requests/2026-08-20-tgb-hunan-prewrite-inspect.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 官方文章 `https://www.tgb.cn/a/2upPwvChvWl`，标题 `8.20湖南人涨停复盘+晚间消息汇总`。
- 官方白底表格原图 `image-01-06.png`，530×5355、910433字节、SHA-256
  `1334a029c37d6148ab0c94b8423f8589118250e71a026e363152bff43dbae9b3`。
- 第一遍题材计数：医药39、机器人6、大消费4、PCB3、算力3、黄金3、其他热点12、其他个股8，合计78；
  市场连板股4行为重复摘要，涨停炸板46行排除。

Deployment:
- 原始证据强刷已执行并更新两份云端日志；正式 TGB 行尚未写入，综合主因未重折，服务未重启。

Notes for next agent:
- 只读检查必须确认正式文件尚不存在、78条候选与过滤终盘池完全一致后，才可进行第二遍原图逐字段
  复核和受保护写入；任一 missing/extra/重复/弱字段或未记录名称差异都必须阻断。

## 2026-08-20 - Codex - TGB 湖南人78行正式写入请求

Changed:
- 受保护只读运行 `32367660735` 确认生产终盘原始池79/79，排除北交所 `920344` 后合格池78/78，
  正式 TGB 文件不存在；人工候选代码集与合格终盘池完全一致。
- 官方原图第二遍逐块、逐行、逐字段人工复核完成；新增加密日期绑定的78行正式写入请求，固定人工
  payload、文章和原图哈希，重验题材计数、代码名称、missing/extra/重复/弱字段，备份后原子写入、
  重折并验证公网状态，任一写后闸失败自动回滚。

Files:
- `.github/workflows/production-ops.yml`
- `ops/production/requests/2026-08-20-tgb-hunan-write.ps1`
- `tests/tgb-20260820-production-request.test.js`
- `docs/DAILY_HANDOFF.md`

Validated:
- 人工 payload：78行/78唯一代码，8个题材块合计78，`missingCodes=[]`、`extraCodes=[]`、重复0、
  `weakCount=0`；名称格式差异仅 `000931` 原图 `中关村`/终盘池 `中 关 村`，NFKC/去空格一致；
  payload SHA-256 `c9dae4e0eab76ad9ecbfdb7a2328ad962c9d6d81cdc64480a91fabed973bbfd5`。
- 终盘池合格代码集 SHA-256 `b5e4260c273f36610701ab88016906a4b5c8f3d6c47ac4b5df6b444bd088b9a4`；
  正式写入前必须保持不变。
- 官方文章 `https://www.tgb.cn/a/2upPwvChvWl`；原图 `image-01-06.png`，910433字节，SHA-256
  `1334a029c37d6148ab0c94b8423f8589118250e71a026e363152bff43dbae9b3`。
- 写前公网为综合归纳、复盘啦、选股宝各78行，韭研0、淘股吧0，`sourceErrors=[]`，`/health` 为
  `ok:true`；日期绑定专项测试、全仓98/98个测试文件与 `git diff --check` 通过。
...
Notes for next agent:
- 只有本日期绑定脚本在 `main` 合并并经 `production` 环境批准后才可执行；公开淘股吧与综合归纳均须
  精确为78、覆盖率100%、低质量0、`sourceErrors=[]`，否则回滚并报告阻断。

## 2026-08-20 - Codex - TGB 湖南人原始证据刷新请求

Changed:
- 确认北京时间 2026-08-20 为星期四交易日；新增日期绑定的受保护生产请求，只强制刷新
  `@TGB湖南人` 官方文章和原始图片证据。
- 请求会先备份同日既有 raw-evidence 目录，并严格校验 manifest 日期、官方文章链接、文章标题以
  `8.20` 开头且包含 `湖南人涨停复盘`、至少一张图片成功下载。

Files:
- `ops/production/requests/2026-08-20-tgb-hunan-raw-evidence.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 公开 2026-08-20 source-view 已有当日综合归纳、复盘啦、选股宝各78行，淘股吧0，
  `sourceErrors=[]`；公开 `/health` 为 `ok:true`。
- 请求只调用 `--tgb-hunan-raw-evidence --day=2026-08-20 --days=1 --force`，不会生成正式行、
  重折综合主因或重启服务。
...
## 2026-08-19 - Codex - TGB 人工转录写前终盘池检查请求

Changed:
- 受保护 raw-evidence 运行 `32248746376` 第四次尝试已保存官方文章及15张原始图片；人工选定
  `image-01-06.png`，排除行情/统计图、同花顺红表、回帖图、头像、小图和炸板区。
- Codex 已按官方原图完成36条正式题材行候选的第一遍逐字段人工录入；新增日期绑定的只读写前检查，
  读取生产终盘池并输出过滤后的代码/名称、排除项和代码集 SHA-256，不写任何文件。

Files:
- `ops/production/requests/2026-08-19-tgb-hunan-prewrite-inspect.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 官方文章 `https://www.tgb.cn/a/2uonRBBKYdH`，标题 `8.19湖南人涨停复盘+晚间消息汇总`。
- 官方白底表格原图 `image-01-06.png`，530×2693、415750字节、SHA-256
  `256192d194c725dd7b2c5502a347e5ea0e0b1982b605bd617bcb983a387bda8a`。
- 第一遍题材计数：房地产7、煤炭5、农业5、医药4、商业航天3、石油化工3、其他热点4、其他个股5，
  合计36；市场连板股7行为重复摘要，涨停炸板12行排除。

Deployment:
- 原始证据强刷已执行并更新两份云端日志；正式 TGB 行尚未写入，综合主因未重折，服务未重启。

Notes for next agent:
- 只读检查必须确认正式文件尚不存在、36条候选与过滤终盘池完全一致后，才可进行第二遍原图逐字段
  复核和受保护写入；任一 missing/extra/重复/弱字段或未记录名称差异都必须阻断。

## 2026-08-19 - Codex - TGB 湖南人36行正式写入请求

Changed:
- 受保护只读运行 `32250085659` 确认生产终盘原始池/过滤池均36/36、无排除项，正式 TGB 文件不存在；
  人工候选代码集与合格终盘池完全一致。
- 官方原图第二遍逐块、逐行、逐字段人工复核完成；新增加密日期绑定的36行正式写入请求，固定人工
  payload、文章和原图哈希，重验题材计数、代码名称、missing/extra/重复/弱字段，备份后原子写入、
  重折并验证公网状态，任一写后闸失败自动回滚。

Files:
- `.github/workflows/production-ops.yml`
- `ops/production/requests/2026-08-19-tgb-hunan-write.ps1`
- `tests/tgb-20260819-production-request.test.js`
- `docs/DAILY_HANDOFF.md`

Validated:
- 人工 payload：36行/36唯一代码，8个题材块合计36，`missingCodes=[]`、`extraCodes=[]`、重复0、
  `weakCount=0`；名称格式差异仅 `000020` 深华发A/深华发Ａ、`002040` 南京港/南 京 港，均为
  NFKC/去空格一致，无实质名称不匹配；payload SHA-256
  `7071183a6774cda893a1edc3b6c4ca9bf487373d6584ed05f46fac6087bc51cd`。
- 终盘池代码集 SHA-256 `7f8be76ecc311913d01373bbaf0e3898f0e0671369e22996050effadd3b6fb07`；
  正式写入前必须保持不变。
- 官方文章 `https://www.tgb.cn/a/2uonRBBKYdH`；原图 `image-01-06.png`，415750字节，
  SHA-256 `256192d194c725dd7b2c5502a347e5ea0e0b1982b605bd617bcb983a387bda8a`。
- 写前公网为综合归纳、复盘啦、选股宝各36行，韭研0、淘股吧0，`sourceErrors=[]`，`/health` 为
  `ok:true`。
...
Changed:
- 受保护 raw-evidence 运行 `32025779601` 已保存官方文章及20张原始图片；人工选定
  `image-01-07.png`，排除统计图、深色行情图、回帖表格、头像、小图、广告、摘要重复和炸板区。
- Codex 已按官方原图完成106条正式题材行候选的第一遍逐字段人工录入；新增日期绑定的只读写前检查，
  读取生产终盘池并输出过滤后的代码/名称、排除项和代码集 SHA-256，不写任何文件。

Files:
- `ops/production/requests/2026-08-17-tgb-hunan-prewrite-inspect.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 官方文章 `https://www.tgb.cn/a/2ukUULM6lx6`，标题 `8.17湖南人涨停复盘+晚间消息汇总`。
- 官方白底表格原图 `image-01-07.png`，530×5727、989124字节、SHA-256
  `42747eedae5809cfb06bbf47d5f358ddc8e72e6bb3e266543f5f4bbc9c17dd03`。
- 第一遍题材计数：机器人12、半导体11、医疗医药10、PCB板9、大消费9、光通信9、算力7、
  农林牧渔6、航天4、燃气轮机4、智能电网4、股权转让3、化工3、液冷服务器3、有色金属3、
  培育钻石2、其他7，合计106；市场连板股15行为重复摘要，涨停炸板12行排除。

Deployment:
- 原始证据强刷已执行并更新两份云端日志；正式 TGB 行尚未写入，综合主因未重折，服务未重启。

Notes for next agent:
- 只读检查必须确认正式文件尚不存在、106条候选与过滤终盘池完全一致后，才可进行第二遍原图逐字段
  复核和受保护写入；任一 missing/extra/重复/弱字段或未记录名称差异都必须阻断。

## 2026-08-17 - Codex - TGB 湖南人106行正式写入请求

Changed:
- 受保护只读运行 `32027180397` 确认生产终盘原始池和过滤池均为106/106、排除项为空、正式 TGB
  文件不存在；人工候选代码集与合格终盘池完全一致。
- 官方原图第二遍逐块、逐行、逐字段人工复核完成；新增加密日期绑定的106行正式写入请求，固定人工
  payload、文章和原图哈希，重验题材计数、代码名称、missing/extra/重复/弱字段，备份后原子写入、
  重折并验证公网状态，任一写后闸失败自动回滚。

Files:
- `.github/workflows/production-ops.yml`
- `ops/production/requests/2026-08-17-tgb-hunan-write.ps1`
- `tests/tgb-20260817-production-request.test.js`
- `docs/DAILY_HANDOFF.md`

Validated:
- 人工 payload：106行/106唯一代码，17个题材块合计106，`missingCodes=[]`、`extraCodes=[]`、重复0、
  `weakCount=0`；4处名称格式差异均为 NFKC/去空格一致，无实质名称不匹配；payload SHA-256
  `715d7f1c2600c043056488c8e23e8f001ad11b22c2af2eac0914750365ac4a6c`。
- 终盘池代码集 SHA-256 `8ba6ebfe6ffcf51c4d0836dd47bde16aea7122f1f506f538518497bddd3f9f3d`；
  正式写入前必须保持不变。
- 官方文章 `https://www.tgb.cn/a/2ukUULM6lx6`；原图 `image-01-07.png`，989124字节，
  SHA-256 `42747eedae5809cfb06bbf47d5f358ddc8e72e6bb3e266543f5f4bbc9c17dd03`。
- 写前公网为综合归纳、复盘啦、选股宝、韭研各106行，淘股吧0，`sourceErrors=[]`。
...
Notes for next agent:
- 只有本日期绑定脚本在 `main` 合并并经 `production` 环境批准后才可执行；公开淘股吧与综合归纳均须
  精确为106、覆盖率100%、低质量0、`sourceErrors=[]`，否则回滚并报告阻断。
...
Changed:
- 受保护 raw-evidence 运行 `31592638969` 已保存官方文章及20张原始图片；人工选定
  `image-01-06.png`，排除同花顺红图、其他作者表格、行情/统计图、头像、小图和账户截图。
- Codex 已按原图11个正式题材块完成91行第一遍逐字段人工转录；新增日期绑定的只读写前检查，
  读取生产终盘池并输出过滤后代码/名称集、排除项和代码集 SHA-256，不写任何文件。

Files:
- `ops/production/requests/2026-08-12-tgb-hunan-prewrite-inspect.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 官方文章 `https://www.tgb.cn/a/2ucBhKEKOG9`，标题 `8.12湖南人涨停复盘+晚间消息汇总`。
- 官方白底表格原图 `image-01-06.png`，530×5068、863142字节、SHA-256
  `23ca24c1aeb3017fe441aafa399ff7b31354048e71078420f4c478a988b0118d`。
- 第一遍题材块计数：大消费13、算力12、光通信10、机器人10、医药7、AI应用5、半导体5、房地产5、
  电力3、其他热点12、其他个股9，合计91；市场连板股16行为重复摘要，涨停炸板11只排除。
...
Changed:
- 受保护原始证据运行 `31097959415` 成功保存 2026-08-06 官方文章和 19 张原始图片；唯一合格
  湖南人白底表格原图为 `image-01-06.png`，其他行情图、同花顺红图、回帖图、头像与个股截图均排除。
- Codex 已按原图人工首遍录入 13 个正式题材块、79 行，并逐字段完成第二遍人工复核；市场连板股重复摘要与
  涨停炸板区均不进入候选。
- 新增日期绑定的只读终盘池检查请求，输出同日原始/过滤计数、排除行、过滤后代码名称和代码集哈希；
  不写文件、不重折综合主因、不重启服务。

Files:
- `ops/production/requests/2026-08-06-tgb-hunan-prewrite-inspect.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 官方文章 `https://www.tgb.cn/a/2u2El7xRP2c`，标题 `8.6湖南人涨停复盘+晚间消息汇总`。
- 原图 `image-01-06.png`：530×5429、891844 字节、SHA-256
  `f5987489d0b923d35bb94f2c64be25ae45eda1e6c53822753e4d0034b47e4799`。
- 人工候选为 79 行/79 唯一代码，13 块合计 79，重复 0、弱字段 0；与公开综合归纳 79 个代码
  初步对照缺失/多余均为空，仍须通过生产终盘池原文件闸后才能正式写入。

Deployment:
- 原始证据刷新已完成，仅改变 raw evidence 与两份云端日志；正式 TGB 行未写入，综合主因未重折，服务未重启。
- 本条只读终盘池检查请求尚未执行。
...
Changed:
- 受保护 raw-evidence 运行 `31002189307` 已保存官方文章及 33 张原始图片；人工选定
  `image-01-06.png`，排除同花顺图、回帖复盘图、头像、小图和炸板区。
- Codex 已完成 102 行人工转录与第二遍逐字段原图复核，未使用 OCR、Qwen 或自动视觉结果。
- 新增日期绑定的只读写前检查，输出终盘池按非 ST、非北交所、非新股前缀过滤后的完整代码/名称集、
  排除项与代码集哈希，不写任何文件。

Files:
- `ops/production/requests/2026-08-05-tgb-hunan-prewrite-inspect.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 官方文章：`https://www.tgb.cn/a/2u10yPnYbir`；官方图片：`image-01-06.png`，长度 1279169，
  SHA-256 `eb61a2164d33fc0e344d4a6f93e66ed690b9bb079ab2325409b395fc4d6e97af`。
- 人工题材块计数：半导体 17、光通信 11、PCB 10、智能驾驶 8、AI液冷散热 7、AI应用 6、
  黄金 6、有色金属 4、磷化铟 3、机器人 4、MLCC 4、算力 4、玻璃基板 3、游戏 2、其他个股 13；合计 102。
- 本地人工转录自检为 102 行/102 唯一代码、重复 0、弱字段 0；终盘池差集尚待受保护只读运行确认。
...
## 2026-08-05 - Codex - TGB 湖南人原始证据刷新请求

Changed:
- 确认北京时间 2026-08-05 为星期三交易日；新增日期绑定的受保护生产请求，只强制刷新
  `@TGB湖南人` 官方文章和原始图片证据。
- 请求会先备份同日既有 raw-evidence 目录，严格校验文章标题、官方链接和已下载图片，且明确禁止
  OCR/Qwen/视觉识别、正式行写入、综合主因重折或服务重启。

Files:
- `ops/production/requests/2026-08-05-tgb-hunan-raw-evidence.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 脚本目标日期固定为 2026-08-05，只调用 `--tgb-hunan-raw-evidence --days=1 --force`。
- 生产结果需在脚本合并并经受保护工作流执行后确认。
...
## 2026-08-06 - Codex - Approve PR #381 and prepare family recut deployment

Changed:
- Independently reviewed PR #381 at head `d93a732` and approved its first trading-family
  repartition, historical family-era freeze, and the #401 async evidence integration.
- Merged PR #381 into `main` as `d3dffc9`.
- Added an atomic production manifest for the main server, current taxonomy, and required
  legacy taxonomy snapshot.

Files:
- `ops/production/manifests/strategy-family-recut-20260806.json`
- `docs/DAILY_HANDOFF.md`

Validated:
- `git diff --check` and `node --check kpl-stats-server.js` passed.
- The legacy taxonomy snapshot is byte-identical to `main@94481a9:theme-taxonomy.json`.
- Family-era, mainline-review, and two-source mainline focused tests passed.
- All **85/85** test files passed on the approved PR head.

Deployment:
- Deployment manifest prepared; production deployment is still pending in this entry.
- The intended deployment window is before the 2026-08-06 09:30 China-market open so
  `STRATEGY_FAMILY_RECUT_EFFECTIVE_DAY=2026-08-06` remains aligned with the first day
  whose artifacts are generated by the new code.

Notes for next agent:
- Deploy all three manifest files atomically. The server intentionally fails fast if the
  legacy taxonomy snapshot is absent or invalid.
- Do not deploy this change during trading hours. If the pre-open window is missed, move
  the effective day to the next trading day and repeat the review before deployment.

## 2026-08-06 - Codex - Deploy PR #381 family recut before market open

Changed:
- Dispatched the reviewed atomic production manifest from exact `main@16c907b`.
- Deployed the main server, current taxonomy, and required legacy taxonomy snapshot as one
  operation before the 2026-08-06 China-market open.
- Restarted only the main Panda Dashboard scheduled task.

Files:
- `kpl-stats-server.js`
- `theme-taxonomy.json`
- `theme-taxonomy.legacy-preB.json`
- `docs/DAILY_HANDOFF.md`

Validated:
- GitHub Actions production run `31061568506` completed successfully.
- Cloud SHA-256 values for all three deployed files exactly match `main@16c907b`.
- `\Panda Dashboard Server` is running and TCP port 8765 is listening.
- Internal `http://127.0.0.1:8765/health` and public
  `https://market.dreamerqi.com/health` both returned HTTP 200.
- Both cloud operation logs contain the successful deployment record.

Deployment:
- Deployed to cloud production at 2026-08-06 09:03:43 +08:00.
- Restarted the main service only.
- Automatic rollback backup:
  `C:\PandaDashboard\_deploy-backups\github-31061568506-1`.

Notes for next agent:
- `STRATEGY_FAMILY_RECUT_EFFECTIVE_DAY=2026-08-06` is now aligned with the first
  trading day whose artifacts are generated by the deployed taxonomy.
- Historical days through 2026-08-05 must continue to resolve through the legacy family
  era; do not rewrite their prediction or frozen snapshot files.

## 2026-08-06 - Codex - Keep unsealed candidates out of review star summaries

Changed:
- Fixed the strategy review summary so the `明星` column only lists stocks confirmed by
  terminal same-day limit-up evidence. An intraday `expected` or `confirmed` level no
  longer promotes a stock after it finishes unsealed.
- Split expanded per-mainline evidence into `确认明星` and `预期跟踪`. Unsealed candidates
  remain visible for audit with their original intraday level and terminal `未封` result,
  but no longer look like confirmed stars.
- Added the exact 2026-07-31 regression: 蓝色光标 remains confirmed, while 捷成股份 and
  天龙集团 are absent from the summary star column and remain under unfulfilled tracking.
...
Validated:
- Production evidence confirms 300182/捷成股份 and 300063/天龙集团 were both `expected`
  with `confirmedAt=null`, and neither appears in `kpl-limitup-db/2026-07-31.json`.
- Replayed the current production review API through the changed renderer: the 07-31
  summary now lists only 蓝色光标、利欧股份、昆仑万维; both unsealed candidates remain
  in the expanded `预期跟踪` section.
- Audited all 22 review days returned by the 30-day production endpoint: zero terminal
  non-sealed stocks leak into the changed summary star column.
- Full repository test suite: **85/85** files passed.

Deployment:
- GitHub only in this entry; not yet merged or deployed. No service was restarted.
- This is a static review-rendering correction. It does not alter prediction files,
  L2 scanning, mainline qualification, review statistics, or runtime databases.

Notes for next agent:
- Preserve the invariant: `predictLevel` is intraday audit evidence; post-close star
...
## 2026-08-07 - Codex - TGB 湖南人原始证据刷新请求

Changed:
- 确认北京时间 2026-08-07 为星期五交易日；新增日期绑定的受保护生产请求，只强制刷新
  `@TGB湖南人` 官方文章和原始图片证据。
- 请求会先备份同日既有 raw-evidence 目录，严格校验文章标题、官方链接和已下载图片，且明确禁止
  OCR/Qwen/视觉识别、正式行写入、综合主因重折或服务重启。

Files:
- `ops/production/requests/2026-08-07-tgb-hunan-raw-evidence.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 目标日期固定为 2026-08-07，只调用 `--tgb-hunan-raw-evidence --days=1 --force`；正式内容仍须等待
  生产原图证据后由 Codex 人工双遍转录和终盘池对账。
...
## 2026-08-07 - Codex - TGB 人工转录写前终盘池检查请求

Changed:
- 受保护 raw-evidence 运行 `31174814530` 已保存官方文章及 15 张原始图片；人工选定
  `image-01-06.png`，排除同花顺/行情图、回帖图、头像和明确的涨停炸板区。
- Codex 已按原图 8 个正式题材块完成 74 行第一遍人工转录与第二遍逐字段人工复核，未使用
  OCR、Qwen 或自动视觉结果；新增日期绑定的只读写前检查，读取生产终盘池并输出过滤后代码/名称集。

Files:
- `ops/production/requests/2026-08-07-tgb-hunan-prewrite-inspect.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 官方文章 `https://www.tgb.cn/a/2u4idr7VS3s`，标题 `8.7湖南人涨停复盘+晚间消息汇总`。
- 官方白底表格原图 `image-01-06.png`，848440 字节，SHA-256
  `046f2ad249f0029aaf7247bb42eeb25e6b0e74a465d31ff569d5e3ad569018cd`。
- 人工题材块计数：医疗医药 20、PCB 18、半导体 8、光通信 3、机器人 3、资产重组 3、
  其他热点 12、其他个股 7，合计 74；市场连板股重复摘要与涨停炸板 26 只均排除。

Deployment:
- 原始证据强刷已执行并写入两份云端日志；正式 TGB 行尚未写入，综合主因未重折，服务未重启。

Notes for next agent:
- 只读检查必须确认正式文件尚不存在，终盘池与 74 行人工候选代码集完全一致后，才可准备受保护写入；
  任一 missing/extra/重复/弱字段或未记录名称差异都必须阻断。

## 2026-08-07 - Codex - TGB 湖南人 74 行正式写入请求

Changed:
- 受保护只读运行 `31177790470` 确认生产终盘原始池和过滤池均为 74/74、排除项为空、正式 TGB
  文件不存在；人工候选与终盘池代码和名称均逐项一致，无名称别名差异。
- 新增加密日期绑定的 74 行正式写入请求：固定人工 payload、官方文章与原图哈希，重验 8 块、代码名称、
  missing/extra/重复/弱字段；写前备份全部受影响文件，原子写入后重折当天综合主因并核验落盘与公开状态，
  任一写后闸失败自动回滚。

Files:
- `.github/workflows/production-ops.yml`
- `ops/production/requests/2026-08-07-tgb-hunan-write.ps1`
- `tests/tgb-20260807-production-request.test.js`
- `docs/DAILY_HANDOFF.md`

Validated:
- 人工 payload：74 行/74 唯一代码，8 块合计 74，`missingCodes=[]`、`extraCodes=[]`、重复 0、
  `weakCount=0`、名称差异 0；SHA-256
  `dc9e5d96824fddcb340991c5bd8705cda55dba5340852343524b4a137315589c`。
- 终盘池代码集 SHA-256 `fa174da8a668baa7513d5fa4b22d9ad7e365b6dc3c8e71484eb18f5efe03e39c`；
  正式文件写入前必须保持不变。
- 官方文章 `https://www.tgb.cn/a/2u4idr7VS3s`；原图 `image-01-06.png`，848440 字节，
  SHA-256 `046f2ad249f0029aaf7247bb42eeb25e6b0e74a465d31ff569d5e3ad569018cd`。
...
Notes for next agent:
- 只有本日期绑定脚本在 `main` 合并并经 `production` 环境批准后才可执行；公开淘股吧与综合归纳均须
  精确为 74/74、覆盖率 100%、低质量 0、`sourceErrors=[]`，否则回滚并报告阻断。

## 2026-08-07 - Codex - TGB 湖南人 74 行正式入库回执

Changed:
- PR #417、#418、#419 已合并；受保护生产运行 `31181477641` 成功写入 2026-08-07
  淘股吧湖南人正式源 74 行，并重折当天综合主因库。
- 正式写入前备份终盘池、正式源、综合/证据/质量/auto、其他三源、原始 manifest/图片和两份云端日志；
  写入与重折后完成落盘、代码集、来源健康和公网接口多重校验，并把成功回执写入两份云端运维日志。

Files:
- Runtime formal source: `C:\PandaDashboard\kpl-limitup-main-reason-sources\tgb-hunan-structured\2026-08-07.json`
- Runtime combined/evidence/quality/auto data for `2026-08-07`
- Cloud logs: `panda-cloud-ops-2026-06-19.md`, `_cloud-change-log-20260705.md`
- `docs/DAILY_HANDOFF.md`
...
- 官方文章 `https://www.tgb.cn/a/2u4idr7VS3s`；原图 `image-01-06.png`，848440 字节，
  SHA-256 `046f2ad249f0029aaf7247bb42eeb25e6b0e74a465d31ff569d5e3ad569018cd`。
- 人工正式行和落盘复核均为 74/74；`missingCodes=[]`、`extraCodes=[]`、`duplicateCodes=[]`、
  `weakCount=0`、名称差异 0，8 个题材块合计 74。
- 综合、证据和 auto TGB 代码集均为 74/74，缺失/多余为空；综合主因已按新正式源完成重折。
- 独立公网复验：综合归纳、复盘啦、选股宝、淘股吧均为 74，韭研为 0；TGB 覆盖率和主因覆盖率
  均为 100%，低质量 0，`sourceErrors=[]`，`/health` 为 `ok:true`。
- 正式文件 SHA-256 `1d84ba4da0ffb49de7427c98da7038ff5324150d9b862a532985e2cd4924f636`；
  综合文件 SHA-256 `00c3267650bbe130bda46d90e24424b58efb5d062c880eab67952622dd3b0b71`。

Deployment:
- Workflow: `https://github.com/dmz1108/dmzrepo/actions/runs/31181477641`。
- 云端备份：`C:\PandaDashboard\backups\tgb-hunan-manual-20260807-20260807131357`。
- 数据写入、综合主因重折和两份云端日志更新已完成；未部署服务代码，未重启服务。
...
## 2026-08-12 - Codex - TGB 湖南人原始证据刷新请求

Changed:
- 确认北京时间 2026-08-12 为星期三交易日；新增日期绑定的受保护生产请求，只强制刷新
  `@TGB湖南人` 官方文章和原始图片证据。
- 请求会先备份同日既有 raw-evidence 目录，并严格校验 manifest 日期、官方文章链接、文章标题以
  `8.12` 开头且包含 `湖南人涨停复盘`、至少一张图片成功下载。

Files:
- `ops/production/requests/2026-08-12-tgb-hunan-raw-evidence.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 公开 2026-08-12 source-view 已有当日综合归纳、复盘啦、选股宝各91行，淘股吧与韭研0，
  `sourceErrors=[]`；公开 `/health` 为 `ok:true`。
- 脚本只调用 `--tgb-hunan-raw-evidence --day=2026-08-12 --days=1 --force`，不会生成正式行、
  重折综合主因或重启服务。
...
## 2026-08-13 - Codex - TGB 湖南人原始证据刷新请求

Changed:
- 确认北京时间 2026-08-13 为星期四交易日；新增日期绑定的受保护生产请求，只强制刷新
  `@TGB湖南人` 官方文章和原始图片证据。
- 请求会先备份同日既有 raw-evidence 目录，并严格校验 manifest 日期、官方文章链接、文章标题以
  `8.13` 开头且包含 `湖南人涨停复盘`、至少一张图片成功下载。

Files:
- `ops/production/requests/2026-08-13-tgb-hunan-raw-evidence.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 公开 2026-08-13 source-view 已有当日综合归纳59行，淘股吧正式源尚未出现，`sourceErrors=[]`；
  公开 `/health` 为 `ok:true`。
- 脚本只调用 `--tgb-hunan-raw-evidence --day=2026-08-13 --days=1 --force`，不会生成正式行、
  重折综合主因或重启服务。
...
- 生产刷新后只可从标题、日期、白底表格及 `@TGB湖南人` 水印均匹配的官方原图继续人工双遍转录；
  若文章、图片或字段不可辨认，必须停止且不得写正式库。
## 2026-08-13 - Codex - TGB 人工转录写前终盘池检查请求

Changed:
- 受保护 raw-evidence 运行 `31696371027` 已保存官方文章及23张原始图片；人工选定
  `image-01-06.png`，排除行情图、统计图、同花顺红图、回帖图、头像、小图与其他来源内容。
- Codex 已按官方原图完成59条正式涨停题材行的第一遍逐字段人工录入；新增日期绑定的只读写前检查，
  读取生产终盘池并输出过滤后的59个代码/名称、排除项和代码集 SHA-256，不写任何文件。

Files:
- `ops/production/requests/2026-08-13-tgb-hunan-prewrite-inspect.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 官方文章 `https://www.tgb.cn/a/2uec4Zbbvhb`，标题 `8.13湖南人涨停复盘+晚间消息汇总`。
- 官方白底表格原图 `image-01-06.png`，530×4592、817029字节、SHA-256
  `90afc9e94f7b2f65d18be2c7760b4b4b7c661b7afa8f27adb1758b9a026d723b`。
- 第一遍题材计数：医药14、算力6、大消费5、机器人5、智能电网5、半导体3、其他热点8、
  其他个股13，合计59；市场连板股21行为重复摘要，涨停炸板35行全部排除。
...
## 2026-08-14 - Codex - TGB 湖南人原始证据刷新请求

Changed:
- 确认北京时间 2026-08-14 为星期五交易日；新增日期绑定的受保护生产请求，只强制刷新
  `@TGB湖南人` 官方文章和原始图片证据。
- 请求会先备份同日既有 raw-evidence 目录，并严格校验 manifest 日期、官方文章链接、文章标题以
  `8.14` 开头且包含 `湖南人涨停复盘`、至少一张图片成功下载。

Files:
- `ops/production/requests/2026-08-14-tgb-hunan-raw-evidence.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 请求只调用 `--tgb-hunan-raw-evidence --day=2026-08-14 --days=1 --force`，不会生成正式行、
  重折综合主因或重启服务。
...
## 2026-08-14 - Codex - TGB 人工转录写前终盘池检查请求

Changed:
- 受保护 raw-evidence 运行 `31796852098` 已保存官方文章及 18 张原始图片；人工选定
  `image-01-06.png`，排除同花顺红图、回帖图、头像、小图、二维码、广告和炸板区。
- Codex 已按官方原图完成 62 条正式题材行候选的第一遍逐字段人工录入；新增日期绑定的只读写前检查，
  读取生产终盘池并输出过滤后的代码/名称、排除项和代码集 SHA-256，不写任何文件。

Files:
- `ops/production/requests/2026-08-14-tgb-hunan-prewrite-inspect.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 官方文章 `https://www.tgb.cn/a/2ug1eFTWduG`，标题 `8.14湖南人涨停复盘+晚间消息汇总`。
- 官方白底表格原图 `image-01-06.png`，530×3995、680547 字节、SHA-256
  `aeb1a552f77c48f819eadd481acd24664aa09afff219d9f1fdcf8d6ef6c51271`。
- 第一遍题材计数：光通信14、医疗医药10、半导体7、机器人5、算力4、生态环保3、其他热点10、
  其他个股9，合计62；市场连板股11行为重复摘要，涨停炸板19行排除。

Deployment:
- 原始证据强刷已执行并更新两份云端日志；正式 TGB 行尚未写入，综合主因未重折，服务未重启。

Notes for next agent:
- 只读检查必须确认正式文件尚不存在、62 条候选与过滤终盘池完全一致后，才可进行第二遍原图逐字段复核
  和受保护写入；任一 missing/extra/重复/弱字段或未记录名称差异都必须阻断。

## 2026-08-14 - Codex - TGB 湖南人62行正式写入请求

Changed:
- 受保护只读运行 `31797860890` 确认生产终盘原始池63/63，排除北交所 `920083` 后合格池62/62，
  正式 TGB 文件不存在；人工候选代码集与合格终盘池完全一致。
- 官方原图第二遍逐块、逐行、逐字段人工复核完成；新增加密日期绑定的62行正式写入请求，固定人工
  payload、文章和原图哈希，重验题材计数、代码名称、missing/extra/重复/弱字段，备份后原子写入、
  重折并验证公网状态，任一写后闸失败自动回滚。

Files:
- `.github/workflows/production-ops.yml`
- `ops/production/requests/2026-08-14-tgb-hunan-write.ps1`
- `tests/tgb-20260814-production-request.test.js`
- `docs/DAILY_HANDOFF.md`

Validated:
- 人工 payload：62行/62唯一代码，8个题材块合计62，`missingCodes=[]`、`extraCodes=[]`、重复0、
  `weakCount=0`；唯一名称差异为 `002081` 源图 `金螳螂` 与终盘池 `金 螳 螂` 的空格规范化差异，
  无实质名称不匹配；payload SHA-256 `f09bc66c7d4c76b30bae9146678cbc85292534d2529cc35cd77f00c8d3ea375c`。
- 终盘池代码集 SHA-256 `dacc4927fac7bb487e4260f5484721e2c98bb2a134711254e63fd662564a235b`；
  正式写入前必须保持不变。
- 官方文章 `https://www.tgb.cn/a/2ug1eFTWduG`；原图 `image-01-06.png`，680547字节，
  SHA-256 `aeb1a552f77c48f819eadd481acd24664aa09afff219d9f1fdcf8d6ef6c51271`。
- 写前公网为综合归纳、复盘啦、选股宝、韭研各62行，淘股吧0，`sourceErrors=[]`，`/health` 为
  `ok:true`；日期绑定专项测试、全仓94/94个测试文件与 `git diff --check` 通过。
...
Notes for next agent:
- 只有本日期绑定脚本在 `main` 合并并经 `production` 环境批准后才可执行；公开淘股吧与综合归纳均须
  精确为62、覆盖率100%、低质量0、`sourceErrors=[]`，否则回滚并报告阻断。

## 2026-08-17 - Codex - TGB 湖南人原始证据刷新请求

Changed:
- 确认北京时间 2026-08-17 为星期一交易日；新增日期绑定的受保护生产请求，只强制刷新
  `@TGB湖南人` 官方文章和原始图片证据。
- 请求会先备份同日既有 raw-evidence 目录，并严格校验 manifest 日期、官方文章链接、文章标题以
  `8.17` 开头且包含 `湖南人涨停复盘`、至少一张图片成功下载。

Files:
- `ops/production/requests/2026-08-17-tgb-hunan-raw-evidence.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 请求只调用 `--tgb-hunan-raw-evidence --day=2026-08-17 --days=1 --force`，不会生成正式行、
  重折综合主因或重启服务。
...
## 2026-08-18 - Codex - TGB 湖南人原始证据刷新请求

Changed:
- 确认北京时间 2026-08-18 为星期二交易日；新增日期绑定的受保护生产请求，只强制刷新
  `@TGB湖南人` 官方文章和原始图片证据。
- 请求会先备份同日既有 raw-evidence 目录，并严格校验 manifest 日期、官方文章链接、文章标题以
  `8.18` 开头且包含 `湖南人涨停复盘`、至少一张图片成功下载。

Files:
- `ops/production/requests/2026-08-18-tgb-hunan-raw-evidence.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 公开 2026-08-18 source-view 已有当日综合归纳、复盘啦、选股宝、韭研各78行，淘股吧0，
  `sourceErrors=[]`；公开 `/health` 为 `ok:true`。
- 请求只调用 `--tgb-hunan-raw-evidence --day=2026-08-18 --days=1 --force`，不会生成正式行、
  重折综合主因或重启服务。
...
## 2026-08-18 - Codex - TGB 人工转录写前终盘池检查请求

Changed:
- 受保护 raw-evidence 运行 `32132714252` 已保存官方文章及17张原始图片；人工选定
  `image-01-06.png`，排除同花顺图、回帖图、头像、小图、广告、市场连板摘要和炸板区。
- Codex 已按官方原图完成78条正式题材行候选的第一遍逐字段人工录入；新增日期绑定的只读写前检查，
  读取生产终盘池并输出过滤后的代码/名称、排除项和代码集 SHA-256，不写任何文件。

Files:
- `ops/production/requests/2026-08-18-tgb-hunan-prewrite-inspect.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 官方文章 `https://www.tgb.cn/a/2umBA1BXLzX`，标题 `8.18湖南人涨停复盘+晚间消息汇总`。
- 官方白底表格原图 `image-01-06.png`，530×4971、847643字节、SHA-256
  `821b8f54a372048365ed450dd0ad65a2093b5a5fc06b0d7484c852cf7a428505`。
- 第一遍题材计数：农林牧渔24、机器人11、大消费8、光通信7、国产软件4、业绩增长4、医疗医药4、
  半导体3、股权转让3、其他热点4、其他个股6，合计78；市场连板股17行为重复摘要，涨停炸板23行排除。
...
## 2026-08-18 - Codex - TGB 湖南人78行正式写入请求

Changed:
- 受保护只读运行 `32133548111` 确认生产终盘原始池79/79，排除北交所 `920087` 后合格池78/78，
  正式 TGB 文件不存在；人工候选代码集与合格终盘池完全一致。
- 官方原图第二遍逐块、逐行、逐字段人工复核完成；新增加密日期绑定的78行正式写入请求，固定人工
  payload、文章和原图哈希，重验题材计数、代码名称、missing/extra/重复/弱字段，备份后原子写入、
  重折并验证公网状态，任一写后闸失败自动回滚。
...
Files:
- `.github/workflows/production-ops.yml`
- `ops/production/requests/2026-08-18-tgb-hunan-write.ps1`
- `tests/tgb-20260818-production-request.test.js`
- `docs/DAILY_HANDOFF.md`

Validated:
- 人工 payload：78行/78唯一代码，11个题材块合计78，`missingCodes=[]`、`extraCodes=[]`、重复0、
  `weakCount=0`；名称格式差异仅 `000020` 深华发A/深华发Ａ、`000735` 罗牛山/罗 牛 山，均为
  NFKC/去空格一致，无实质名称不匹配；payload SHA-256
  `e63932951881de0e35116562ccb9df0446b4c833e901356a30484874528327c9`。
- 终盘池代码集 SHA-256 `95d077a3454d40645e797be627452dc1ac868d96e99083d08b80d6753e99cd8e`；
  正式写入前必须保持不变。
- 官方文章 `https://www.tgb.cn/a/2umBA1BXLzX`；原图 `image-01-06.png`，847643字节，
  SHA-256 `821b8f54a372048365ed450dd0ad65a2093b5a5fc06b0d7484c852cf7a428505`。
- 写前公网为综合归纳、复盘啦、选股宝、韭研各78行，淘股吧0，`sourceErrors=[]`，`/health` 为
  `ok:true`；日期绑定专项测试、全仓96/96个测试文件与 `git diff --check` 通过。
...
## 2026-08-19 - Codex - TGB 湖南人原始证据刷新请求

Changed:
- 确认北京时间 2026-08-19 为星期三交易日；新增日期绑定的受保护生产请求，只强制刷新
  `@TGB湖南人` 官方文章和原始图片证据。
- 请求会先备份同日既有 raw-evidence 目录，并严格校验 manifest 日期、官方文章链接、文章标题以
  `8.19` 开头且包含 `湖南人涨停复盘`、至少一张图片成功下载。

Files:
- `ops/production/requests/2026-08-19-tgb-hunan-raw-evidence.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 公开 2026-08-19 source-view 已有当日复盘啦、选股宝各36行，淘股吧0，`sourceErrors=[]`；
  公开 `/health` 为 `ok:true`。
- 请求只调用 `--tgb-hunan-raw-evidence --day=2026-08-19 --days=1 --force`，不会生成正式行、
  重折综合主因或重启服务。
...
## 2026-08-20 - Codex - TGB 湖南人正式入库与公网验收回执

Changed:
- PR #463、#464、#465 已依次合并；受保护运行 `32364722634` 强刷官方 raw evidence，
  `32367660735` 只读对账终盘池，`32377889128` 完成78行正式写入与当日综合主因重折。
- 正式写入前已备份同日终盘池、综合/证据/质量/auto 产物、已健康来源、raw manifest/原图与两份云端日志；
  写入后两份云端运维日志已追加安全回执。
...
Validated:
- 官方文章 `https://www.tgb.cn/a/2upPwvChvWl`，原图 `image-01-06.png`，530×5355、910433字节，
  SHA-256 `1334a029c37d6148ab0c94b8423f8589118250e71a026e363152bff43dbae9b3`。
- 官方原图人工双遍复核78行：医药39、机器人6、大消费4、PCB3、算力3、黄金3、其他热点12、
  其他个股8；市场连板股4行为摘要重复，涨停炸板46行排除。
- 终盘原始池79/79，排除北交所 `920344` 后78/78；人工候选与落盘后二次闸均为
  `missing=[]`、`extra=[]`、重复0、`weak=0`、题材合计78、实质名称不匹配0；仅 `000931`
  `中关村`/`中 关 村` 为已声明的 NFKC/去空格差异。
- 正式 TGB SHA-256 `5e253a1fae85825c5d2b9cebd7d82725100de8a2c8c6d5ed48c8c5c44b57a705`；
  重折综合 SHA-256 `1085f1aac393fa515f394722cc0dc4c27bd68dca81e8a085f9e11fff5900dcaa`。
- 独立公网复验：综合归纳、复盘啦、选股宝、淘股吧均78/78，韭研0；TGB 覆盖率/主因覆盖率100%，
  低质量0，`sourceErrors=[]`，`/health` 为 `ok:true`。

Deployment:
- 生产数据已写入并重折当日综合主因；备份路径
  `C:\\PandaDashboard\\backups\\tgb-hunan-manual-20260820-20260820141041`。
- 未部署服务代码，未重启主服务或其他服务。

Notes for next agent:
- 2026-08-20 正式 TGB 文件已受人工来源保护；普通同步和 `force` 不得覆盖。若需更正，须重做备份、
  全量对账、重折和公网验收。

## 2026-08-21 - Codex - TGB 湖南人原始证据刷新请求

Changed:
- 确认北京时间 2026-08-21 为星期五交易日；新增日期绑定的受保护生产请求，只强制刷新
  `@TGB湖南人` 官方文章和原始图片证据。
- 请求会先备份同日既有 raw-evidence 目录，并严格校验 manifest 日期、官方文章链接、文章标题以
  `8.21` 开头且包含 `湖南人涨停复盘`、至少一张图片成功下载。

Files:
- `ops/production/requests/2026-08-21-tgb-hunan-raw-evidence.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 请求只调用 `--tgb-hunan-raw-evidence --day=2026-08-21 --days=1 --force`，不会生成正式行、
  重折综合主因或重启服务。
...
## 2026-08-24 - Codex - TGB 湖南人原始证据刷新请求

Changed:
- 确认北京时间 2026-08-24 为星期一交易日；新增日期绑定的受保护生产请求，只强制刷新
  `@TGB湖南人` 官方文章和原始图片证据。
- 请求会先备份同日既有 raw-evidence 目录，并严格校验 manifest 日期、官方文章链接、文章标题以
  `8.24` 开头且包含 `湖南人涨停复盘`、至少一张图片成功下载。

Files:
- `ops/production/requests/2026-08-24-tgb-hunan-raw-evidence.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 公开 2026-08-24 source-view 已有当日综合归纳、复盘啦、选股宝各46行，淘股吧0，
  `sourceErrors=[]`；公开 `/health` 为 `ok:true`。
- 请求只调用 `--tgb-hunan-raw-evidence --day=2026-08-24 --days=1 --force`，不会生成正式行、
  重折综合主因或重启服务。
...
## 2026-08-24 - Codex - TGB 人工转录写前终盘池检查请求

Changed:
- 受保护 raw-evidence 运行 `32722742773` 已保存官方文章及17张原始图片；人工选定
  `image-01-06.png`，排除同花顺红图、深色行情/统计图、回帖图、头像、小图、市场连板摘要和炸板区。
- Codex 已按官方原图完成46条正式题材行候选的第一遍逐字段人工录入；新增日期绑定的只读写前检查，
  读取生产终盘池并输出过滤后的代码/名称、排除项和代码集 SHA-256，不写任何文件。

Files:
- `ops/production/requests/2026-08-24-tgb-hunan-prewrite-inspect.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 官方文章 `https://www.tgb.cn/a/2uwthn6EfR5`，标题 `8.24湖南人涨停复盘+晚间消息汇总`。
- 官方白底表格原图 `image-01-06.png`，530×3366、559319字节、SHA-256
  `ca7433198be529fa7c75567e501b88e6d59aece70dde1c5b2f2b3b26e46f464f`。
- 第一遍题材计数：医药6、锂电池4、半导体3、电力3、黄金3、机器人3、煤炭3、算力3、
  其他热点14、其他个股4，合计46；市场连板股10行为重复摘要，涨停炸板16行排除。

Deployment:
- 原始证据强刷已执行并更新两份云端日志；正式 TGB 行尚未写入，综合主因未重折，服务未重启。

Notes for next agent:
- 只读检查必须确认正式文件尚不存在、46条候选与过滤终盘池完全一致后，才可进行第二遍原图逐字段
  复核和受保护写入；任一 missing/extra/重复/弱字段或未记录名称差异都必须阻断。

## 2026-08-24 - Codex - TGB 湖南人46行正式写入请求

Changed:
- 受保护只读运行 `32723903119` 确认生产终盘原始池/合格池均46/46，无排除项，正式 TGB 文件
  不存在；人工候选代码集与合格终盘池完全一致。
- 官方原图第二遍逐块、逐行、逐字段人工复核完成；新增加密日期绑定的46行正式写入请求，固定人工
  payload、文章和原图哈希，重验题材计数、代码名称、missing/extra/重复/弱字段，备份后原子写入、
  重折并验证公网状态，任一写后闸失败自动回滚。

Files:
- `.github/workflows/production-ops.yml`
- `ops/production/requests/2026-08-24-tgb-hunan-write.ps1`
- `tests/tgb-20260824-production-request.test.js`
- `docs/DAILY_HANDOFF.md`

Validated:
- 人工 payload：46行/46唯一代码，10个题材块合计46，`missingCodes=[]`、`extraCodes=[]`、重复0、
  `weakCount=0`；唯一名称格式差异为 `000931` 中关村/中 关 村，NFKC/去空格一致；payload
  SHA-256 `5111ba4722637df742609d8c04251e77a1a68f4bb37fb1433c55f4e43cbaa675`。
- 终盘池代码集 SHA-256 `9d622810fb2e496038750242470224453bad78cb2a003d22a606ad0fca47619c`；
  正式写入前必须保持不变。
- 官方文章 `https://www.tgb.cn/a/2uwthn6EfR5`；原图 `image-01-06.png`，559319字节，
  SHA-256 `ca7433198be529fa7c75567e501b88e6d59aece70dde1c5b2f2b3b26e46f464f`。
...
Notes for next agent:
- 只有本日期绑定脚本在 `main` 合并并经 `production` 环境批准后才可执行；公开淘股吧与综合归纳均须
  精确为46、覆盖率100%、低质量0、`sourceErrors=[]`，否则回滚并报告阻断。

## 2026-08-25 - Codex - TGB 湖南人原始证据刷新请求

Changed:
- 确认北京时间 2026-08-25 为星期二交易日；新增日期绑定的受保护生产请求，只强制刷新
  `@TGB湖南人` 官方文章和原始图片证据。
- 请求会先备份同日既有 raw-evidence 目录，并严格校验 manifest 日期、官方文章链接、文章标题以
  `8.25` 开头且包含 `湖南人涨停复盘`、至少一张图片成功下载。

Files:
- `ops/production/requests/2026-08-25-tgb-hunan-raw-evidence.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 公开 2026-08-25 source-view 已有当日综合归纳、复盘啦、选股宝各65行，淘股吧0，
  `sourceErrors=[]`；公开 `/health` 为 `ok:true`。
- 请求只调用 `--tgb-hunan-raw-evidence --day=2026-08-25 --days=1 --force`，不会生成正式行、
  重折综合主因或重启服务。
...
## 2026-08-25 - Codex - TGB 人工转录写前终盘池检查请求

Changed:
- 受保护 raw-evidence 运行 `32843257305` 已保存官方文章及29张原始图片；人工选定
  `image-01-06.png`，排除同花顺红图、深色行情/统计图、回帖图、头像、小图、市场连板摘要和炸板区。
- Codex 已按官方原图完成65条正式题材行候选的第一遍逐字段人工录入；新增日期绑定的只读写前检查，
  读取生产终盘池并输出过滤后的代码/名称、排除项和代码集 SHA-256，不写任何文件。

Files:
- `ops/production/requests/2026-08-25-tgb-hunan-prewrite-inspect.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 官方文章 `https://www.tgb.cn/a/2uy7Jgoa8l6`，标题 `8.25湖南人涨停复盘+晚间消息汇总`。
- 官方白底表格原图 `image-01-06.png`，530×4158、719980字节、SHA-256
  `44ad0bdd1001811e65b0645e0bf99f885807dad494b1f8fa631e4a029cfdb6e4`。
- 第一遍题材计数：医药9、大消费6、液冷5、国产芯片5、机器人4、农业4、算力4、股权转让3、
  其他热点14、其他个股11，合计65；市场连板股8行为重复摘要，涨停炸板22行排除。

Deployment:
- 原始证据强刷已执行并更新两份云端日志；正式 TGB 行尚未写入，综合主因未重折，服务未重启。

Notes for next agent:
- 只读检查必须确认正式文件尚不存在、65条候选与过滤终盘池完全一致后，才可进行第二遍原图逐字段
  复核和受保护写入；任一 missing/extra/重复/弱字段或未记录名称差异都必须阻断。

## 2026-08-25 - Codex - TGB 湖南人65行正式写入请求

Changed:
- 受保护只读运行 `32844761403` 确认生产终盘原始池/合格池均65/65，无排除项，正式 TGB 文件
  不存在；人工候选代码集与合格终盘池完全一致。
- 官方原图第二遍逐块、逐行、逐字段人工复核完成；新增加密日期绑定的65行正式写入请求，固定人工
  payload、文章和原图哈希，重验题材计数、代码名称、missing/extra/重复/弱字段，备份后原子写入、
  重折并验证公网状态，任一写后闸失败自动回滚。

Files:
- `.github/workflows/production-ops.yml`
- `ops/production/requests/2026-08-25-tgb-hunan-write.ps1`
- `tests/tgb-20260825-production-request.test.js`
- `docs/DAILY_HANDOFF.md`

Validated:
- 人工 payload：65行/65唯一代码，10个题材块合计65，`missingCodes=[]`、`extraCodes=[]`、重复0、
  `weakCount=0`；唯一名称格式差异为 `002081` 金螳螂/金 螳 螂，NFKC/去空格一致；payload
  SHA-256 `da828edb16f9045181f06a89a446b3601bce76183f5d57799ee04d8d730914a1`。
- 终盘池代码集 SHA-256 `944a93e07c53750e36715643f59cd95d94973eece424dd4def5528d39b434f26`；
  正式写入前必须保持不变。
- 官方文章 `https://www.tgb.cn/a/2uy7Jgoa8l6`；原图 `image-01-06.png`，719980字节，
  SHA-256 `44ad0bdd1001811e65b0645e0bf99f885807dad494b1f8fa631e4a029cfdb6e4`。
...
## 2026-08-26 - Codex - TGB 湖南人原始证据刷新请求

Changed:
- 确认北京时间 2026-08-26 为星期三交易日；新增日期绑定的受保护生产请求，只强制刷新
  `@TGB湖南人` 官方文章和原始图片证据。
- 请求会先备份同日既有 raw-evidence 目录，并严格校验 manifest 日期、官方文章链接、文章标题以
  `8.26` 开头且包含 `湖南人涨停复盘`、至少一张图片成功下载。

Files:
- `ops/production/requests/2026-08-26-tgb-hunan-raw-evidence.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 公开 2026-08-26 source-view 已有当日综合归纳、复盘啦、选股宝、韭研各52行，淘股吧0，
  `sourceErrors=[]`；公开 `/health` 为 `ok:true`。
- 请求只调用 `--tgb-hunan-raw-evidence --day=2026-08-26 --days=1 --force`，不会生成正式行、
  重折综合主因或重启服务。
...
## 2026-08-27 - Codex - TGB 人工转录写前终盘池检查请求

Changed:
- 受保护 raw-evidence 运行 `33131697844` 已保存官方文章及23张原始图片；人工选定
  `image-01-06.png`，排除同花顺红图、统计图、回帖图、头像、小图、市场连板摘要和炸板区。
- Codex 已按官方原图完成76条正式题材行候选的第一遍逐字段人工录入；新增日期绑定的只读写前检查，
  读取生产终盘池并输出过滤后的代码/名称、排除项和代码集 SHA-256，不写任何文件。

Files:
- `ops/production/requests/2026-08-27-tgb-hunan-prewrite-inspect.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 官方文章 `https://www.tgb.cn/a/2uBuay3MSqq`，标题 `8.27湖南人涨停复盘+晚间消息汇总`。
- 官方白底表格原图 `image-01-06.png`，530×4736、801096字节、SHA-256
  `18e03a34538b496c7982d6c6c52a78d694ade3b45e9f86ea262a264d2b5ebf2c`。
- 第一遍题材计数：光通信15、农林牧渔8、PCB6、大金融5、半导体4、氟化工4、液冷服务器4、
  医疗医药4、存储3、黄金3、业绩增长3、其他热点12、其他个股5，合计76；市场连板股16行为摘要重复，
  涨停炸板17行排除。
...
## 2026-08-28 - Codex - TGB 湖南人原始证据刷新请求

Changed:
- 确认北京时间 2026-08-28 为星期五交易日；新增日期绑定的受保护生产请求，只强制刷新
  `@TGB湖南人` 官方文章和原始图片证据。
- 请求会先备份同日既有 raw-evidence 目录，并严格校验 manifest 日期、官方文章链接、文章标题以
  `8.28` 开头且包含 `湖南人涨停复盘`、至少一张图片成功下载。

Files:
- `ops/production/requests/2026-08-28-tgb-hunan-raw-evidence.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 公开 2026-08-28 source-view 当前综合归纳、复盘啦、选股宝各81行，韭研与淘股吧均0，
  `sourceErrors=[]`；公开 `/health` 为 `ok:true`。
- 请求只调用 `--tgb-hunan-raw-evidence --day=2026-08-28 --days=1 --force`，不会生成正式行、
  重折综合主因或重启服务。
...
## 2026-08-31 - Codex - TGB 湖南人补录原始证据刷新请求

Changed:
- 根据 Owner 补做北京时间 2026-08-31 星期一交易日复盘；新增日期绑定的受保护生产请求，只强制刷新
  `@TGB湖南人` 官方文章和原始图片证据。
- 请求会先备份同日既有 raw-evidence 目录，并严格校验 manifest 日期、官方文章链接、文章标题以
  `8.31` 开头且包含 `湖南人涨停复盘`、至少一张图片成功下载。

Files:
- `ops/production/requests/2026-08-31-tgb-hunan-raw-evidence.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 公开 2026-08-31 source-view 当前综合归纳、复盘啦、选股宝、韭研各86行，淘股吧0，
  `sourceErrors=[]`；公开 `/health` 为 `ok:true`。
- 请求只调用 `--tgb-hunan-raw-evidence --day=2026-08-31 --days=1 --force`，不会生成正式行、
  重折综合主因或重启服务。
...
Notes for next agent:
- 强刷后只可选择标题、日期、白底表格及 `@TGB湖南人` 水印均匹配的官方原图；若文章、图片或字段
  不可辨认，必须停止且不得写正式库。

## 2026-08-31 - Codex - TGB 人工转录写前终盘池检查请求

Changed:
- 受保护 raw-evidence 运行 `33477569455` 已保存官方文章及38张原始图片；人工选定
  `image-01-06.png`，排除小图、同花顺/行情图、回帖图、头像、市场连板摘要和涨停炸板区。
- Codex 已按官方原图完成86条正式题材行候选的第一遍逐字段人工录入；新增日期绑定的只读写前检查，
  读取生产终盘池并输出过滤后的代码/名称、排除项和代码集 SHA-256，不写任何文件。

Files:
- `ops/production/requests/2026-08-31-tgb-hunan-prewrite-inspect.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 官方文章 `https://www.tgb.cn/a/2uI68eHSbbC`，标题 `8.31湖南人涨停复盘+晚间消息汇总`。
- 官方白底表格原图 `image-01-06.png`，530×5658、1018426字节、SHA-256
  `5aeb7988fe8276f399a0afed0f5f49a8dcae6715141e9b6c95be40e8a3784116`。
- 第一遍题材计数：短剧16、AI液冷10、房地产7、机器人8、PCB6、半导体5、大消费5、业绩增长5、
  农业4、算力4、化工3、其他热点6、其他个股7，13块合计86；市场连板股18行为重复摘要，
  涨停炸板30行排除。
...
Notes for next agent:
- 只读检查必须确认正式文件尚不存在、86条候选与过滤终盘池完全一致后，才可进行第二遍原图逐字段
  复核和受保护写入；任一 missing/extra/重复/弱字段或未记录名称差异都必须阻断。

## 2026-08-31 - Codex - TGB 湖南人86行正式写入请求

Changed:
- 受保护只读运行 `33478441862` 确认生产终盘原始池88/88，排除北交所 `920021`、`920223` 后
  合格池86/86，正式 TGB 文件不存在；人工候选代码集与合格终盘池完全一致。
- 官方原图第二遍逐块、逐行、逐字段人工复核完成；新增加密日期绑定的86行正式写入请求，固定人工
  payload、文章和原图哈希，重验题材计数、代码名称、missing/extra/重复/弱字段，备份后原子写入、
  重折并验证公网状态，任一写后闸失败自动回滚。

Files:
- `.github/workflows/production-ops.yml`
- `ops/production/requests/2026-08-31-tgb-hunan-write.ps1`
- `tests/tgb-20260831-production-request.test.js`
- `docs/DAILY_HANDOFF.md`

Validated:
- 人工 payload：86行/86唯一代码，13个题材块合计86，`missingCodes=[]`、`extraCodes=[]`、重复0、
  `weakCount=0`；唯一名称格式差异为 `002081` 金螳螂/金 螳 螂，NFKC/去空格一致；payload
  SHA-256 `a435ae415c7bad6fe59246f674d637ffd4fa24ffbb352d75cadf6a144e46fee4`。
- 终盘池合格代码集 SHA-256 `46becb80c15a990912eecd2e96ca8643733e72384398a7ca2ab0b8a7b47f9e44`；
  正式写入前必须保持不变。
- 官方文章 `https://www.tgb.cn/a/2uI68eHSbbC`；原图 `image-01-06.png`，1018426字节，
  SHA-256 `5aeb7988fe8276f399a0afed0f5f49a8dcae6715141e9b6c95be40e8a3784116`。
...
Notes for next agent:
- 只有本日期绑定脚本在 `main` 合并并经 `production` 环境批准后才可执行；公开淘股吧与综合归纳均须
  精确为86、覆盖率100%、低质量0、`sourceErrors=[]`，否则回滚并报告阻断。

## 2026-09-01 - Codex - TGB 湖南人人工转录写前终盘池检查请求

Changed:
- 受保护 raw-evidence 运行 `33512947854` 已成功强刷官方文章及27张原始图片；人工选定
  `image-01-06.png`，排除同花顺红图、行情/统计图、回帖图、头像、二维码、广告、市场连板摘要和涨停炸板区。
- Codex 已按官方原图完成80条正式题材行候选的第一遍逐字段人工录入；新增日期绑定的只读写前检查，
  读取生产终盘池并输出过滤后的代码/名称、排除项和代码集 SHA-256，不写任何文件。

Files:
- `ops/production/requests/2026-09-01-tgb-hunan-prewrite-inspect.ps1`
- `docs/DAILY_HANDOFF.md`

Validated:
- 官方文章 `https://www.tgb.cn/a/2uJJ0MqzjQV`，标题 `9.1湖南人涨停复盘+晚间消息汇总`。
- 官方白底表格原图 `image-01-06.png`，530x4698、773689字节、SHA-256
  `74a95606968f8ee71894cfc44b021758a7bec7a4305521071f4048fd53e2de14`。
- 第一遍题材计数：农业14、大消费12、短剧10、医药8、大金融7、AI应用4、光通信3、无人驾驶3、其他热点8、其他个股11，
  10块合计80；市场连板股18行为重复摘要，涨停炸板6行排除。
...
## 2026-09-14 - Codex - TGB 湖南人55行正式入库完成

Changed:
- 按北京时间 2026-09-14 星期一交易日执行每日复盘；通过直接云端受保护流程强制刷新
  `@TGB湖南人` 官方文章，22张引用图中21张下载成功；旧同日 raw 目录不存在，因此无旧 raw 备份。
- Codex 只查看官方原图，逐题材块、逐行、逐字段完成第一遍人工转录和第二遍人工复核；第一遍把
  `601123` 误看成“马钢股份”，第二遍回看原图后更正为清晰可辨的“马矿股份”，且与终盘池一致。
  全程未使用 Qwen、OCR 或任何自动视觉结果生成、补全、猜测或校验正式行。
- 受保护写入流程备份全部受影响运行时文件，写入55行正式 TGB 来源并重折当天综合主因库；两份
  云端运维日志均已记录，远端一次性脚本和 payload 已清理并确认不存在。

Files:
- `docs/DAILY_HANDOFF.md`
- 生产运行时：`C:\PandaDashboard\kpl-limitup-main-reason-sources\tgb-hunan-raw\2026-09-14\`
- 生产运行时：`C:\PandaDashboard\kpl-limitup-main-reason-sources\tgb-hunan-structured\2026-09-14.json`
- 生产运行时：`C:\PandaDashboard\kpl-limitup-main-reason-db\2026-09-14.json`
- 云端日志：`C:\PandaDashboard\panda-cloud-ops-2026-06-19.md`
- 云端日志：`C:\PandaDashboard\_cloud-change-log-20260705.md`

Validated:
- 官方文章 `https://www.tgb.cn/a/2v5lwUBLQwY`，标题 `9.14湖南人涨停复盘+晚间消息汇总`；选定
  官方白底表格 `image-01-06.png`，530×4185、726297字节、SHA-256
  `8701025fa88947de552e39ec2e55e6290c10b6c33c4a639e58e93d09861a1d24`，标题日期和
  `@TGB湖南人` 水印均匹配。
- 排除头像、小图、行情统计图、同花顺红色可视化图、回帖图、二维码、广告、11行重复“市场连板股”
  摘要以及30行“涨停炸板”；正式题材计数为PCB 7、大消费6、AI安全5、医药医疗5、电力4、
  新能源汽车4、半导体3、光通信3、机器人3、算力3、其他8、其他个股4，合计55。
- 终盘原始池55/55唯一，无排除项，合格池55/55；合格代码集 SHA-256
  `91cb52801076b04291cf7ca9f19ab47520b4142dfc762a57ae1e46c721806ad7`。正式候选55/55唯一，
  `missingCodes=[]`、`extraCodes=[]`、重复0、`weakCount=0`、名称差异0；人工 payload SHA-256
  `d569d701f2262479d7418b6ab1e099fbe440ee7e9a81d4b4fb6619be4c4073f6`。
- 成功写入备份 `C:\PandaDashboard\backups\tgb-hunan-manual-20260914-20260914141227`；正式文件
  SHA-256 `7b98142384d9c9571eb5291293a7f810e5f38d68226079072b6fd46c11f0e784`，综合主因库
  SHA-256 `e78372541d818218ff456897c60449722a00c726d981ef5787de52cf86debe92`。
- 独立公网复验：综合归纳55、复盘啦55、选股宝55、韭研0、淘股吧55；TGB覆盖率100%、主因覆盖率
  100%、低置信0，`sourceErrors=[]`；综合主因当天55行、复盘覆盖率100%、多源覆盖率100%，公开
  `/health` 为 `ok:true`。
...
Notes for next agent:
- 2026-09-14 正式文件已受人工来源保护，普通同步与 `force` 不得覆盖；若需更正，必须先备份并重新执行
  官方原图人工双遍复核及全部终盘池质量闸。
