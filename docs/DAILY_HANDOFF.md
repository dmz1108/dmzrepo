...
## 2026-09-15 - Codex - TGB 湖南人32行正式入库完成

Changed:
- 通过日期绑定的受保护生产流程强刷 `2026-09-15` 官方原文和21张原始图片；同日旧 raw 目录不存在，
  因此无需旧 raw 备份。OCR、Qwen 和自动结构化全程禁用。
- Codex 人工选定官方白底分组表 `image-01-06.png`，逐题材块、逐行、逐字段完成32行正式内容的
  第一遍录入和第二遍人工复核；排除头像/小图、统计图、同花顺红图、回帖图、7行市场连板摘要和
  24行涨停炸板区。
- 受保护写入流程备份全部受影响运行时文件，原子写入32行正式 TGB 来源，重折当日综合主因库，
  并把安全回执追加到两份云端运维日志；远端一次性脚本和载荷已删除并确认不存在。

Files:
- `docs/DAILY_HANDOFF.md`

Validated:
- 官方文章 `https://www.tgb.cn/a/2v6YqAlrkrc`，标题 `9.15湖南人涨停复盘+晚间消息汇总`；选定原图
  `image-01-06.png` 为530x2726、456354字节，SHA-256
  `37639d272c74c461d588232e44a9d05a39592370160b2ab4eff29d2709c0866d`，标题、日期和
  `@TGB湖南人` 水印均匹配。
- 六个正式题材块：风电6、PCB5、半导体5、固态电池3、其他热点8、其他个股5，合计32。
- 终盘原始池/合格池/正式行均32/32，无排除项；`missingCodes=[]`、`extraCodes=[]`、重复0、
  `weakCount=0`、名称差异0；合格代码集 SHA-256
  `b095df99c701d8e504b7f56239c6855a69adad37d42fb46546dfb430adcd17a9`，人工载荷 SHA-256
  `ab8815ab5178236eeec93a051920bb931e9046f858e92ba43416593e48ef7dc7`。
- 生产备份 `C:\PandaDashboard\backups\tgb-hunan-manual-20260915-20260915114518`；正式 TGB SHA-256
  `e7c53facb342deb5ee9a03d022c39c5411c7247bc2850749de6c64dff71d2dfc`；综合主因 SHA-256
  `a4637b0351a7827d8103965d19f673810c5d215bf5d37f35df01212d42ccdabf`。
- 独立公网复验：综合归纳32、复盘啦32、选股宝32、韭研0、淘股吧32；TGB覆盖率/主因覆盖率100%，
  低置信0，`sourceErrors=[]`；综合库32行，复盘与多源覆盖率均100%，`/health` 为 `ok:true`。

Deployment:
- 已写入云端正式 TGB 运行时数据并重折当日综合主因库；未部署应用代码，未重启主站、Caddy、
  娱乐服务、SSH 或公司端 L2 worker。

Notes for next agent:
- `2026-09-15` 正式 TGB 文件现受人工来源保护；普通同步和 `force` 不得覆盖。如需更正，必须先备份、
  重做完整人工双遍复核和全部对账闸，再受控替换并重折。
