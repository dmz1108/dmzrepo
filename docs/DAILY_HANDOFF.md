...
## 2026-08-28 - Codex - TGB 湖南人正式入库与公网验收回执

Changed:
- PR #488、#489、#490、#491 已依次合并；受保护运行 `33168057953` 强刷官方 raw evidence，
  `33168894814` 只读对账终盘池，`33170059188` 完成81行正式写入与当日综合主因重折。
- 首次写入运行 `33169802270` 因未预声明 `000565` 全角/半角名称差异在写入前安全阻断；修正后完整
  重跑通过。正式写入前已备份同日终盘池、综合/证据/质量/auto 产物、已健康来源、raw manifest/
  原图与两份云端日志；写入后两份云端运维日志已追加安全回执。

Files:
- `docs/DAILY_HANDOFF.md`

Validated:
- 官方文章 `https://www.tgb.cn/a/2uD6GIwPL0s`，原图 `image-01-06.png`，530×4905、876211字节，
  SHA-256 `8e2f1542914c01777d25014f5b3d77cc86b6434783637d8d335f23392b6eca74`。
- 官方原图人工双遍复核81行：农业12、AI应用8、机器人5、算力5、医药5、大消费4、房地产4、
  国产芯片4、智能电网4、化工4、氟化工/PTFE3、其他热点12、其他个股11；市场连板股17行为摘要重复，
  涨停炸板16行排除。
- 终盘原始池82/82，排除北交所 `920895` 后81/81；人工候选与落盘后二次闸均为
  `missing=[]`、`extra=[]`、重复0、`weak=0`、题材合计81、实质名称不匹配0；仅 `000565`
  渝三峡A/渝三峡Ａ、`002165` 红宝丽/红 宝 丽为显式 NFKC 格式差异。
- 正式 TGB SHA-256 `6293eb400bf85b1b122e00ff4518db26b11e474e31f2f23ed4e19e53a45b5b80`；
  重折综合 SHA-256 `7917f3764707d1479683749a701aa832e123ff271d65fbe7b8c530cc631b5efe`。
- 独立公网复验：综合归纳、复盘啦、选股宝、淘股吧均81，韭研0；TGB 覆盖率/主因覆盖率100%，
  低质量0，`sourceErrors=[]`，`/health` 为 `ok:true`。

Deployment:
- 生产数据已写入并重折当日综合主因；备份路径
  `C:\\PandaDashboard\\backups\\tgb-hunan-manual-20260828-20260828121144`。
- 未部署服务代码，未重启主服务或其他服务。

Notes for next agent:
- 2026-08-28 正式 TGB 文件已受人工来源保护；普通同步和 `force` 不得覆盖。若需更正，须重做备份、
  全量对账、重折和公网验收。
