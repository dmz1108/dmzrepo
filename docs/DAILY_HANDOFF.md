...
## 2026-08-26 - Codex - TGB 湖南人正式入库与公网验收回执

Changed:
- PR #480、#481、#482 已依次合并；受保护运行 `32964832189` 强刷官方 raw evidence，
  `32966313434` 只读对账终盘池，`32967586866` 完成52行正式写入与当日综合主因重折。
...
Validated:
- 官方文章 `https://www.tgb.cn/a/2uzSn8RLuWv`，原图 `image-01-06.png`，530×3783、644390字节，
  SHA-256 `32a6d6c2651fe43ed5eece818ae0ad4f0064f22aac58b47fe6f30997ae4725c8`。
- 官方原图人工双遍复核52行：大金融6、光通信6、公告5、医药5、黄金4、半导体3、可控核聚变3、农业3、
  有色金属3、其他热点6、其他个股8；市场连板股13行为重复摘要，涨停炸板20行排除。
- 终盘原始池/过滤池均52/52；人工候选与落盘后二次闸均为 `missing=[]`、`extra=[]`、重复0、
  `weak=0`、题材合计52、实质名称不匹配0；仅 `000997` 新大陆/新 大 陆为已声明的去空格差异。
- 正式 TGB SHA-256 `737124d8c2feabb0cf7ed47fa4adf9db0fb5c0bc56739e32f99edef5a4e8f4c0`；
  重折综合 SHA-256 `8ab89233956889dc757c01d8ac099102c7888425a56fd2deb0a2ed5656d0eeef`。
- 独立公网复验：综合归纳、复盘啦、选股宝、韭研、淘股吧均52；TGB覆盖率/主因覆盖率100%，
  低置信0，`sourceErrors=[]`，`/health` 为 `ok:true`。

Deployment:
- 生产数据已写入并重折当日综合主因；备份路径
  `C:\\PandaDashboard\\backups\\tgb-hunan-manual-20260826-20260826121558`。
- 未部署服务代码，未重启主服务或其他服务。

Notes for next agent:
- 2026-08-26 正式 TGB 文件已受人工来源保护；普通同步和 `force` 不得覆盖。若需更正，须重做备份、
  全量对账、重折和公网验收。
